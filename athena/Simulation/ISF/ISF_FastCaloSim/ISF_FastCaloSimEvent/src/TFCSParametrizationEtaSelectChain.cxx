...
int TFCSParametrizationEtaSelectChain::get_bin( TFCSSimulationState&, const TFCSTruthState*,
                                                const TFCSExtrapolationState* extrapol ) const {
  return val_to_bin( extrapol->IDCaloBoundary_eta() );
}

const std::string TFCSParametrizationEtaSelectChain::get_variable_text( TFCSSimulationState&, const TFCSTruthState*,
                                                                        const TFCSExtrapolationState* extrapol ) const {
...
const std::string TFCSParametrizationEtaSelectChain::get_bin_text( int bin ) const {
  if ( bin == -1 || bin >= (int)get_number_of_bins() ) {
    return std::string(
        Form( "bin=%d not in [%2.2f<=eta<%2.2f)", bin, m_bin_low_edge[0], m_bin_low_edge[get_number_of_bins()] ) );
  }
  return std::string( Form( "bin=%d, %2.2f<=eta<%2.2f", bin, m_bin_low_edge[bin], m_bin_low_edge[bin + 1] ) );
}

void TFCSParametrizationEtaSelectChain::unit_test( TFCSSimulationState* simulstate, TFCSTruthState* truth,
