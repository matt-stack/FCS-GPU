...
TFCSSimulationState::TFCSSimulationState( CLHEP::HepRandomEngine* randomEngine ) : m_randomEngine( randomEngine ) {
...
void TFCSSimulationState::Print( Option_t* ) const {
  ...
  for ( int i = 0; i < CaloCell_ID_FCS::MaxSample; ++i )
    if ( E( i ) != 0 ) {
      std::cout << "  E" << i << "(" << CaloSampling::getSamplingName( i ) << ")=" << E( i ) << " E" << i
                << "/E=" << Efrac( i ) << std::endl;
    }
}
