...
TFCSLateralShapeParametrization::TFCSLateralShapeParametrization( const char* name, const char* title )
    : TFCSParametrization( name, title ), m_Ekin_bin( -1 ), m_calosample( -1 ) {}

void TFCSLateralShapeParametrization::set_Ekin_bin( int bin ) { m_Ekin_bin = bin; }

void TFCSLateralShapeParametrization::set_calosample( int cs ) { m_calosample = cs; }
...
void TFCSLateralShapeParametrization::Print( Option_t* option ) const {
  ...
  if ( longprint ) {
    if ( Ekin_bin() == -1 )
      ...
      ATH_MSG_INFO( optprint << "  Ekin_bin=" << Ekin_bin() << " ; calosample=" << calosample() );
  }
}
