...
void TFCSExtrapolationState::Print( Option_t* ) const {
  std::cout << "IDCalo: eta=" << m_IDCaloBoundary_eta << " phi=" << m_IDCaloBoundary_phi << " r=" << m_IDCaloBoundary_r
            << " z=" << m_IDCaloBoundary_z << std::endl;
  for ( int i = 0; i < CaloCell_ID_FCS::MaxSample; ++i ) {
    if ( m_CaloOK[i][SUBPOS_MID] ) {
      std::cout << "  layer " << i << " MID eta=" << m_etaCalo[i][SUBPOS_MID] << " phi=" << m_phiCalo[i][SUBPOS_MID]
                << " r=" << m_rCalo[i][SUBPOS_MID] << " z=" << m_zCalo[i][SUBPOS_MID] << std::endl;
    }
  }
}
