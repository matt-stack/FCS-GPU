...
TFCSEnergyInterpolationSpline::TFCSEnergyInterpolationSpline( const char* name, const char* title )
    : TFCSParametrization( name, title ) {}

void TFCSEnergyInterpolationSpline::InitFromArrayInLogEkin( Int_t np, Double_t logEkin[], Double_t response[],
                                                            const char* opt, Double_t valbeg, Double_t valend ) {
  TSpline3 initspline( GetName(), logEkin, response, np, opt, valbeg, valend );
  m_spline = initspline;
}

void TFCSEnergyInterpolationSpline::InitFromArrayInEkin( Int_t np, Double_t Ekin[], Double_t response[],
                                                         const char* opt, Double_t valbeg, Double_t valend ) {
...
FCSReturnCode TFCSEnergyInterpolationSpline::simulate( TFCSSimulationState& simulstate, const TFCSTruthState* truth,
  ...
  if ( logEkin < m_spline.GetXmin() ) {
    ...
    if ( logEkin > m_spline.GetXmax() ) {
      Emean = ( m_spline.Eval( m_spline.GetXmax() ) +
                m_spline.Derivative( m_spline.GetXmax() ) * ( logEkin - m_spline.GetXmax() ) ) *
              truth->Ekin();
    } else {
...
void TFCSEnergyInterpolationSpline::Print( Option_t* option ) const {
  ...
  if ( longprint )
    ATH_MSG_INFO( optprint << "  Spline N=" << m_spline.GetNp() << " " << m_spline.GetXmin()
                           << "<=log(Ekin)<=" << m_spline.GetXmax() << " " << TMath::Exp( m_spline.GetXmin() )
                           << "<=Ekin<=" << TMath::Exp( m_spline.GetXmax() ) );
}

void TFCSEnergyInterpolationSpline::unit_test( TFCSSimulationState* simulstate, TFCSTruthState* truth,
                                               const TFCSExtrapolationState* extrapol, TGraph* grspline ) {
  ...
  if ( !grspline ) {
    ...
    Double_t  Graph0_fx1001[Graph0_n] = {1.024, 2.048, 4.094, 8.192, 16.384, 32.768, 65.536, 131.072, 262.144};
    for ( int i = 0; i < Graph0_n; ++i ) Graph0_fx1001[i] *= 1000;

    Double_t Graph0_fy1001[Graph0_n] = {0.6535402, 0.6571529, 0.6843001, 0.7172835, 0.7708416,
                                        0.798819,  0.8187628, 0.8332745, 0.8443931};
  ...
  */
  ...
  for ( float Ekin = test.Ekin_min() * 0.25; Ekin <= test.Ekin_max() * 4; Ekin *= 1.05 ) {
    ...
    if ( test.simulate( *simulstate, truth, extrapol ) != FCSSuccess ) { return; }
