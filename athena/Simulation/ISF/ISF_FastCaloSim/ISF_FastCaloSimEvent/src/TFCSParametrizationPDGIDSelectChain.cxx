  ...
  for ( auto param : chain() ) {
    ...
    if ( param->is_match_pdgid( truth->pdgid() ) ) {
      ATH_MSG_DEBUG( "pdgid=" << truth->pdgid() << ", now run: " << param->GetName()
                              << ( ( SimulateOnlyOnePDGID() == true ) ? ", abort PDGID loop afterwards"
...
void TFCSParametrizationPDGIDSelectChain::unit_test( TFCSSimulationState* simulstate, TFCSTruthState* truth,
