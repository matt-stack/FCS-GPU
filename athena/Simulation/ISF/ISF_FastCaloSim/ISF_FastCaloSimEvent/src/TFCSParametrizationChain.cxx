...
void TFCSParametrizationChain::recalc_pdgid_intersect() {
  ...
  for ( auto param : m_chain ) {
    ...
    std::set_intersection( pdgid().begin(), pdgid().end(), param->pdgid().begin(), param->pdgid().end(),
...
void TFCSParametrizationChain::recalc_pdgid_union() {
  ...
  for ( auto param : chain() ) {
    ...
    std::set_union( pdgid().begin(), pdgid().end(), param->pdgid().begin(), param->pdgid().end(),
                    std::inserter( tmp, tmp.begin() ) );
    set_pdgid( tmp );
  }
}
...
bool TFCSParametrizationChain::is_match_Ekin_bin( int Ekin_bin ) const {
  for ( auto param : m_chain )
    if ( !param->is_match_Ekin_bin( Ekin_bin ) ) return false;
  return true;
}

bool TFCSParametrizationChain::is_match_calosample( int calosample ) const {
  for ( auto param : m_chain )
    if ( !param->is_match_calosample( calosample ) ) return false;
  return true;
}

FCSReturnCode TFCSParametrizationChain::simulate( TFCSSimulationState& simulstate, const TFCSTruthState* truth,
                                                  const TFCSExtrapolationState* extrapol ) {
  for ( auto param : m_chain ) {
    if ( simulate_and_retry( param, simulstate, truth, extrapol ) != FCSSuccess ) { return FCSFatal; }
  }
...
void TFCSParametrizationChain::Streamer( TBuffer& R__b ) {
  ...
  if ( R__b.IsReading() ) {
    ...
    if ( R__v == 1 ) {
      ...
      if ( R__b.GetParent() ) {
        if ( parent->InheritsFrom( TDirectory::Class() ) ) { dir = (TDirectory*)parent; }
      }
        ...
        if ( R__t != nullptr ) {
          if ( R__t->InheritsFrom( TFCSParametrizationPlaceholder::Class() ) ) {
            ...
            if ( new_R__t ) {
              ...
              Error( "TFCSParametrizationChain::Streamer", "Found placeholder object in the parametrization chain, but "
                                                           "could not read the real object from the file!" );
            }
          }
        }
    ...
    if ( R__n ) {
      ...
      for ( R__k = R__stl.begin(); R__k != R__stl.end(); ++R__k ) {
        ...
        if ( dir && R__t != nullptr ) {
          ...
          new_R__t =
              new TFCSParametrizationPlaceholder( R__t->GetName(), TString( "Placeholder for: " ) + R__t->GetTitle() );
          R__t = new_R__t;
        }
        R__b << R__t;
