...
int TFCSParametrizationBinnedChain::get_bin( TFCSSimulationState&, const TFCSTruthState*,
                                             const TFCSExtrapolationState* ) const {
  return 0;
}

const std::string TFCSParametrizationBinnedChain::get_variable_text( TFCSSimulationState&, const TFCSTruthState*,
                                                                     const TFCSExtrapolationState* ) const {
...
FCSReturnCode TFCSParametrizationBinnedChain::simulate( TFCSSimulationState& simulstate, const TFCSTruthState* truth,
                                                        const TFCSExtrapolationState* extrapol ) {
  ATH_MSG_DEBUG( "In BinnedChain, sub Chain size " << chain().size() );
  for ( auto ichain : chain() )
    ATH_MSG_DEBUG( "Chain[" << ichain << "] name:" << typeid( *( ichain ) ).name() << " Title=" << ichain->GetName() );
  ...
  for ( unsigned int ichain = 0; ichain < m_bin_start[0]; ++ichain ) {
    ...
    if ( simulate_and_retry( chain()[ichain], simulstate, truth, extrapol ) != FCSSuccess ) { return FCSFatal; }
  }
  if ( get_number_of_bins() > 0 ) {
    int bin = get_bin( simulstate, truth, extrapol );
    ATH_MSG_DEBUG( "Number of bins:  " << get_number_of_bins() << " bin " << bin );
    if ( bin >= 0 && bin < (int)get_number_of_bins() ) {
      for ( unsigned int ichain = m_bin_start[bin]; ichain < m_bin_start[bin + 1]; ++ichain ) {
        ATH_MSG_DEBUG( "for " << get_variable_text( simulstate, truth, extrapol ) << " run " << get_bin_text( bin )
    ...
    for ( unsigned int ibin = 0; ibin < get_number_of_bins(); ++ibin ) {
      if ( ichain == m_bin_start[ibin] ) {
        if ( ibin < get_number_of_bins() - 1 )
          if ( ichain == m_bin_start[ibin + 1] ) continue;
        prefix = Form( "%-2d", ibin );
...
void TFCSParametrizationBinnedChain::unit_test( TFCSSimulationState* simulstate, const TFCSTruthState* truth,
                                                const TFCSExtrapolationState* extrapol ) {
