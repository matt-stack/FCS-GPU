...
double TFCS1DFunctionSpline::Initialize( TH1* hist, double maxdevgoal, double maxeffsiggoal, int maxnp ) {
  ...
  for ( int np = 3; np <= maxnp; ++np ) {
    ...
    std::cout << "========== Spline #=" << np << " max_penalty_best=" << max_penalty_best
              << " ==============" << std::endl;
...
double TFCS1DFunctionSpline::InitializeFromSpline( TH1* hist, const TSpline3& sp, double maxdevgoal,
  ...
  std::cout << "Spline init spline #=" << nsplinepoints << " : maxdev=" << maxdev << " p_maxdev=" << p_maxdev
            << " maxeffsig=" << maxeffsig << " p_maxeffsig=" << p_maxeffsig << " max_penalty=" << max_penalty
            << std::endl;
  std::cout << "  ";
...
double TFCS1DFunctionSpline::InitializeEqualDistance( TH1* hist, double maxdevgoal, double maxeffsiggoal,
                                                      int nsplinepoints ) {
  TFCS1DFunctionInt32Histogram hist_fct( hist );
  ...
  std::cout << "Spline init equ. dist. #=" << nsplinepoints << " : maxdev=" << maxdev << " p_maxdev=" << p_maxdev
            << " maxeffsig=" << maxeffsig << " p_maxeffsig=" << p_maxeffsig << " max_penalty=" << max_penalty
            << std::endl;
  std::cout << "  ";
  for ( int i = 0; i < m_spline.GetNp(); ++i ) {
...
double TFCS1DFunctionSpline::InitializeEqualProbability( TH1* hist, double maxdevgoal, double maxeffsiggoal,
                                                         int nsplinepoints ) {
  TFCS1DFunctionInt32Histogram hist_fct( hist );

  double              dprop = 1.0 / ( nsplinepoints - 1 );
  std::vector<double> nprop( nsplinepoints );
  for ( int i = 0; i < nsplinepoints; ++i ) { nprop[i] = i * dprop; }
  ...
  double p_maxeffsig;
  double maxdev = get_maxdev( hist, m_spline, maxeffsig, p_maxdev, p_maxeffsig );
  std::cout << "Spline init equ. prob. #=" << nsplinepoints << " : maxdev=" << maxdev << " p_maxdev=" << p_maxdev
            << " maxeffsig=" << maxeffsig << " p_maxeffsig=" << p_maxeffsig << " max_penalty=" << max_penalty
            << std::endl;
  std::cout << "  ";
  for ( int i = 0; i < m_spline.GetNp(); ++i ) {
...
double TFCS1DFunctionSpline::optimize( TSpline3& sp_best, std::vector<double>& nprop, const TH1* hist,
                                       TFCS1DFunctionInt32Histogram& hist_fct, double maxdevgoal,
                                       double maxeffsiggoal ) {
  ...
  for ( double dproploop = 0.4 / nsplinepoints; dproploop > 0.02 / nsplinepoints; dproploop /= 2 ) {
    ...
    for ( int itry = 0; itry < ntry; ++itry ) {
      itrytot++;
      for ( int i = 0; i < nsplinepoints; ++i ) { nx[i] = hist_fct.rnd_to_fct( nprop_try[i] ); }
...
double TFCS1DFunctionSpline::get_maxdev( const TH1* hist, const TSpline3& sp, double& maxeffsig, double& p_maxdev,
  ...
  for ( int i = 0; i <= hist->GetNbinsX() + 1; i++ ) {
    ...
    // std::cout<<i<<": diff="<<int1-int2<<" sig(diff)="<<valsig<<" int1="<<int1<<" +- "<<int1err<<" int2="<<int2<<"
    // maxdev="<<maxdev<<" maxeffsig="<<maxeffsig<<std::endl;
  }
...
double TFCS1DFunctionSpline::rnd_to_fct( double rnd ) const { return m_spline.Eval( rnd ); }
