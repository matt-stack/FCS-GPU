...
TFCSPCAEnergyParametrization::TFCSPCAEnergyParametrization( const char* name, const char* title )
...
FCSReturnCode TFCSPCAEnergyParametrization::simulate( TFCSSimulationState& simulstate, const TFCSTruthState* /*truth*/,
  ...
  if ( pcabin == 0 ) {
    ...
    for ( unsigned int i = 0; i < m_RelevantLayers.size(); i++ ) layerNr.push_back( m_RelevantLayers[i] );
    ...
    for ( unsigned int l = 0; l < layerNr.size(); l++ ) { simdata[l] *= scalefactor; }
    ...
    for ( int s = 0; s < CaloCell_ID_FCS::MaxSample; s++ ) {
      ...
      for ( unsigned int l = 0; l < layerNr.size(); l++ ) {
        if ( layerNr[l] == s ) energyfrac = simdata[l];
      }
...
void TFCSPCAEnergyParametrization::P2X( TVectorD* SigmaValues, TVectorD* MeanValues, TMatrixD* EV, int gNVariables,
                                        double* p, double* x, int nTest ) {
...
bool TFCSPCAEnergyParametrization::loadInputs( TFile* file ) { return loadInputs( file, "" ); }
...
bool TFCSPCAEnergyParametrization::loadInputs( TFile* file, std::string folder ) {
  ...
  for ( int bin = 1; bin <= m_numberpcabins; bin++ ) {
    ...
    if ( symCov == NULL ) {
      ATH_MSG_WARNING( "TFCSPCAEnergyParametrization::symCov in pcabin " << bin << " is null!" );
      load_ok = false;
    }
    if ( MeanValues == NULL ) {
      ATH_MSG_WARNING( "TFCSPCAEnergyParametrization::MeanValues in pcabin " << bin << " is null!" );
      load_ok = false;
    }
    if ( SigmaValues == NULL ) {
      ATH_MSG_WARNING( "TFCSPCAEnergyParametrization::SigmaValues in pcabin " << bin << " is null!" );
      load_ok = false;
    }
    if ( Gauss_means == NULL ) {
      ATH_MSG_WARNING( "TFCSPCAEnergyParametrization::Gauss_means in pcabin " << bin << " is null!" );
      load_ok = false;
    }
    if ( Gauss_rms == NULL ) {
      ATH_MSG_WARNING( "TFCSPCAEnergyParametrization::Gause_rms in pcabin " << bin << " is null!" );
      load_ok = false;
    }
    ...
    for ( unsigned int l = 0; l < layer.size(); l++ ) {
      ...
      if ( !fct ) fct = (TFCS1DFunction*)gDirectory->Get( "TFCS1DFunctionRegressionTF" );
      if ( !fct ) fct = (TFCS1DFunction*)gDirectory->Get( "TFCS1DFunctionHistogram" );
...
void TFCSPCAEnergyParametrization::clean() {
  for ( unsigned int i = 0; i < m_EV.size(); i++ ) delete m_EV[i];
}
