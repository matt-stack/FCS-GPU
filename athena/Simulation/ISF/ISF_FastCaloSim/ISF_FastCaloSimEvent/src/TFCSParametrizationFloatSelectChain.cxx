...
int TFCSParametrizationFloatSelectChain::push_back_in_bin( TFCSParametrizationBase* param, float low, float up ) {
  ...
  if ( ilow < 0 && iup < 0 ) {
    ...
    ATH_MSG_ERROR( "Cannot add " << param->GetName() << ": range [" << low << "," << up
                                 << ") which is outside existing range [" << m_bin_low_edge[0] << ","
  ...
  ATH_MSG_ERROR( "Cannot add " << param->GetName() << ": range [" << low << "," << up
                               << ") covers more than one bin in existing range [" << m_bin_low_edge[0] << ","
                               << m_bin_low_edge[get_number_of_bins()] << ") or splits an existing bin" );
  return -1;
}
...
void TFCSParametrizationFloatSelectChain::unit_test( TFCSSimulationState*, TFCSTruthState*,
