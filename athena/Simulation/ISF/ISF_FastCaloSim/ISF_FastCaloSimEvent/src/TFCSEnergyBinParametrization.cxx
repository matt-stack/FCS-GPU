...
TFCSEnergyBinParametrization::TFCSEnergyBinParametrization( const char* name, const char* title )
...
void TFCSEnergyBinParametrization::set_pdgid_Ekin_bin_probability( int id, std::vector<float> prob ) {
  ...
  if ( prob.size() != m_pdgid_Ebin_probability[id].size() ) {
    ATH_MSG_ERROR(
        "TFCSEnergyBinParametrization::set_pdgid_Ekin_bin_probability(): size of vectors does not match! in.size()="
...
void TFCSEnergyBinParametrization::load_pdgid_Ekin_bin_probability_from_file( int id, TFile* file,
  ...
  if ( !pcabinprobvector ) {
    ...
    for ( unsigned int i = 1; i < m_pdgid_Ebin_probability[id].size(); i++ ) { prob[i] = 1.0; }
  }

  if ( pcabinprobvector ) {
    if ( (unsigned int)pcabinprobvector->GetNoElements() != m_pdgid_Ebin_probability[id].size() ) {
      ATH_MSG_ERROR( "TFCSEnergyBinParametrization::load_pdgid_Ekin_bin_probability_from_file(): size of prob array "
                     "does not match! in.size()="
                     << pcabinprobvector->GetNoElements() << " instance=" << m_pdgid_Ebin_probability[id].size() );
      return;
    }
...
FCSReturnCode TFCSEnergyBinParametrization::simulate( TFCSSimulationState& simulstate, const TFCSTruthState* truth,
  ...
  if ( chosenBin < 0 || chosenBin > n_bins() ) {
    ...
    if ( msgLvl( MSG::ERROR ) ) {
      ATH_MSG( ERROR ) << "in " << GetName() << ": E=" << simulstate.E() << " Ebin=" << chosenBin
                       << " rnd=" << searchRand << " array=";
