...
ISF_FCS_Parametrization::FCS_StepInfo::operator+=( const ISF_FCS_Parametrization::FCS_StepInfo& other ) {
  if ( identify() != other.identify() ) {
    std::cout << "Warning: Not merging hits from different cells!!! " << identify() << " / " << other.identify()
              << std::endl;
    return *this;
  }
