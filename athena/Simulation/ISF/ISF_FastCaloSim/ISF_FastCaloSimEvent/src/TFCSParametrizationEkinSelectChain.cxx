...
int TFCSParametrizationEkinSelectChain::get_bin( TFCSSimulationState& simulstate, const TFCSTruthState* truth,
  ...
  if ( Ekin < first_in_bin->Ekin_nominal() ) {
    ...
    ATH_MSG_DEBUG( "logEkin=" << logEkin << " logEkin_previous=" << logEkin_previous
                              << " logEkin_nominal=" << logEkin_nominal << " (rnd=" << 1 - rnd
                              << " < p(previous)=" << ( 1 - numerator / denominator ) << ")? => orgbin=" << prevbin + 1
                              << " selbin=" << bin );
  } else {
    ...
    ATH_MSG_DEBUG( "logEkin=" << logEkin << " logEkin_nominal=" << logEkin_nominal << " logEkin_next=" << logEkin_next
                              << " (rnd=" << rnd << " < p(next)=" << numerator / denominator
...
const std::string TFCSParametrizationEkinSelectChain::get_variable_text( TFCSSimulationState&,
...
const std::string TFCSParametrizationEkinSelectChain::get_bin_text( int bin ) const {
  if ( bin == -1 || bin >= (int)get_number_of_bins() ) {
    return std::string(
        Form( "bin=%d not in [%1.1f<=Ekin<%1.1f)", bin, m_bin_low_edge[0], m_bin_low_edge[get_number_of_bins()] ) );
  }
  if ( DoRandomInterpolation() ) {
    return std::string(
        Form( "bin=%d, %1.1f<=Ekin(+random)<%1.1f", bin, m_bin_low_edge[bin], m_bin_low_edge[bin + 1] ) );
  }
  return std::string( Form( "bin=%d, %1.1f<=Ekin<%1.1f", bin, m_bin_low_edge[bin], m_bin_low_edge[bin + 1] ) );
}

void TFCSParametrizationEkinSelectChain::unit_test( TFCSSimulationState* simulstate, TFCSTruthState* truth,
