...
TFCS1DFunctionHistogram::TFCS1DFunctionHistogram( TH1* hist, double cut_maxdev ) { Initialize( hist, cut_maxdev ); }

void TFCS1DFunctionHistogram::Initialize( TH1* hist, double cut_maxdev ) { smart_rebin_loop( hist, cut_maxdev ); }
...
TH1* TFCS1DFunctionHistogram::vector_to_histo() {
  ...
  for ( unsigned int i = 0; i < m_HistoBorders.size(); i++ ) bins[i] = m_HistoBorders[i];

  TH1* h_out = new TH1D( "h_out", "h_out", m_HistoBorders.size() - 1, bins );
  for ( int b = 1; b <= h_out->GetNbinsX(); b++ ) h_out->SetBinContent( b, m_HistoContents[b - 1] );
...
void TFCS1DFunctionHistogram::smart_rebin_loop( TH1* hist, double cut_maxdev ) {
  ...
  while ( 1 ) {
    ...
    if ( i % 100 == 0 )
      cout << "Iteration nr. " << i << " -----> change " << change << " bins " << h_out->GetNbinsX()
           << " -> maxdev=" << maxdev << endl;
  ...
  for ( int b = 1; b <= h_output->GetNbinsX(); b++ ) m_HistoBorders.push_back( (float)h_output->GetBinLowEdge( b ) );
  m_HistoBorders.push_back( (float)h_output->GetXaxis()->GetXmax() );

  for ( int b = 1; b < h_output->GetNbinsX(); b++ ) m_HistoContents.push_back( h_output->GetBinContent( b ) );
...
TH1D* TFCS1DFunctionHistogram::smart_rebin( TH1D* h_input ) {
  ...
  for ( int b = 1; b < h_out1->GetNbinsX() - 1; b++ ) // never touch the last bin
  {
    ...
    if ( diff <= change && !merged ) {
      ...
      merged = 1;
      skip   = 1;
      if ( b == ( h_out1->GetNbinsX() - 2 ) ) secondlastbin_merge = 1;
    }
  } // for b
  ...
  for ( unsigned int i = 0; i < binborder.size(); i++ ) bins[i] = binborder[i];

  TH1D* h_out2 = new TH1D( "h_out2", "h_out2", content.size(), bins );
  for ( unsigned int b = 1; b <= content.size(); b++ ) h_out2->SetBinContent( b, content[b - 1] );
