...
void TFCS1DFunction::rnd_to_fct( float value[], const float rnd[] ) const { value[0] = rnd_to_fct( rnd[0] ); }
...
double TFCS1DFunction::get_maxdev( TH1* h_input1, TH1* h_approx1 ) {
  ...
  for ( int i = 1; i <= h_input->GetNbinsX(); i++ ) {
    double val = fabs( h_approx->GetBinContent( h_approx->FindBin( h_input->GetBinCenter( i ) ) ) -
...
double TFCS1DFunction::CheckAndIntegrate1DHistogram( const TH1* hist, std::vector<double>& integral_vec, int& first,
                                                     int& last ) {
  ...
  for ( int ix = 1; ix <= nbins; ix++ ) {
    ...
    if ( binval < 0 ) {
      ...
      if ( TMath::Abs( fraction ) > 1e-5 ) {
        std::cout << "WARNING: bin content is negative in histogram " << hist->GetName() << " : " << hist->GetTitle()
                  << " binval=" << binval << " " << fraction * 100 << "% of integral=" << hist->Integral()
  ...
  for ( first = 0; first < nbins; first++ )
    if ( integral_vec[first] != 0 ) break;
  for ( last = nbins - 1; last > 0; last-- )
    if ( integral_vec[last] != integral ) break;
  last++;

  if ( integral <= 0 ) {
    std::cout << "ERROR: histogram " << hist->GetName() << " : " << hist->GetTitle() << " integral=" << integral
              << " is <=0" << std::endl;
  }
  return integral;
}
...
TH1* TFCS1DFunction::generate_histogram_random_gauss( int nbinsx, int ntoy, double xmin, double xmax, double xpeak,
                                                      double sigma ) {
