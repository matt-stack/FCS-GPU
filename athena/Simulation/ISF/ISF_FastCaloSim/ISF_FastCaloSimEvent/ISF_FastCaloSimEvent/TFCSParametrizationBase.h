...
namespace MSG {
  enum Level { NIL = 0, VERBOSE, DEBUG, INFO, WARNING, ERROR, FATAL, ALWAYS, NUM_LEVELS }; // enum Level
  __attribute__( ( unused ) ) static const char* LevelNames[NUM_LEVELS] = {"NIL",     "VERBOSE", "DEBUG", "INFO",
...
#  define ATH_MSG_LVL_NOCHK( lvl, x )                                                                                  \
    do {                                                                                                               \
      if ( this->msgLvl( lvl ) )                                                                                       \
        this->msg( lvl ) << std::setw( 45 ) << std::left << this->GetName() << " " << MSG::LevelNames[lvl] << " " << x \
                         << endmsg;                                                                                    \
    } while ( 0 )
...
#  define ATH_MSG( lvl )                                                                                               \
    ...
    this->msg( MSG::lvl ) << std::setw( 45 ) << std::left << this->GetName() << " " << MSG::LevelNames[MSG::lvl] << " "
...
Functionality in derivde classes is  provided through the simulate method. The simulate method takes a TFCSTruthState
and a TFCSExtrapolationState object as input and provides output in a TFCSSimulationState. Parametrizations contain
information on the pdgid, range in Ekin and range in eta of particles to which they can be applied. Several basic types
of parametrization exists:
- classes derived from TFCSEnergyParametrization simulate energy information which is written into TFCSSimulationState
- classes derived from TFCSLateralShapeParametrization simulate cell level information for specific calorimeter layers
and bins "Ebin" in the energy parametrization
- classes derived from TFCSParametrizationChain call other parametrization. Depending on the derived class, these other
parametrization are only called under special conditions
- a special case of TFCSLateralShapeParametrization is TFCSLateralShapeParametrizationHitBase for hit level shape
simulation through the simulate_hit method. Hit level simulation is controlled through the special chain
...
public:
  ...
  virtual void setLevel( int level, bool recursive = false ) {
    ...
    if ( recursive )
      for ( unsigned int i = 0; i < size(); ++i ) ( *this )[i]->setLevel( m_level, recursive );
  }
