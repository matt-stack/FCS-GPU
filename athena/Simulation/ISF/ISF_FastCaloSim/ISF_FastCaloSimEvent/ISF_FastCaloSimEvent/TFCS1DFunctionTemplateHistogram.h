...
public:
  ...
  /// set number of bins.
  /// The actualy alocated size is one smaller than count, as the last bin is fixed with the range
...
public:
  ...
  TFCS1DFunctionTemplateHistogram( TH1* hist = nullptr ) {
  ...
  /// Initialize from root histogram. Depending on the precision of the x- and y-axis, bins are merged if numerical
  /// identical
      ...
      for ( Int_t ibin = first; ibin <= last; ++ibin ) {
        ...
        if ( ihist > 0 )
          if ( m_HistoBorders.GetBinLowEdge( ihist - 1 ) == m_HistoBorders.GetBinLowEdge( ihist ) ) {
            std::cout << "Skip bin=" << ibin + 1 << " x=" << hist->GetXaxis()->GetBinLowEdge( ibin + 1 )
                      << " fx=" << m_HistoBorders.GetBinLowEdge( ihist ) << std::endl;
            --ihist;
            std::cout << "     bin=" << ibin << " x=" << hist->GetXaxis()->GetBinLowEdge( ibin )
                      << " fx=" << m_HistoBorders.GetBinLowEdge( ihist ) << std::endl;
          }
        ...
        if ( ihist > 0 )
          if ( m_HistoContents.get_fraction( ihist - 1 ) == m_HistoContents.get_fraction( ihist ) ) {
            std::cout << "Skip fbin=" << ihist << " fx=" << m_HistoBorders.GetBinLowEdge( ihist )
                      << " frac=" << m_HistoContents.get_fraction( ihist ) << std::endl;
            --ihist;
            std::cout << "     fbin=" << ihist << " fx=" << m_HistoBorders.GetBinLowEdge( ihist )
                      << " frac=" << m_HistoContents.get_fraction( ihist ) << std::endl;
          }

        std::cout << "bin=" << ibin + 1 << " fbin=" << ihist << "/" << m_HistoBorders.get_nbins() << " x=["
                  << hist->GetXaxis()->GetBinLowEdge( ibin + 1 ) << "," << hist->GetXaxis()->GetBinUpEdge( ibin + 1 )
                  << "] fx=" << m_HistoBorders.GetBinLowEdge( ihist ) << " int=" << temp_HistoContents[ibin] / integral
                  << " frac=" << m_HistoContents.get_fraction( ihist ) << std::endl;
  ...
  virtual double rnd_to_fct( double rnd ) const {
    ...
    // frac="<<m_HistoContents.get_fraction(ibin)<<" residual_rnd="<<residual_rnd<<std::endl;
...
template <typename Txvec, typename Ty, typename Trandom = float>
class TFCS1DFunctionTemplateInterpolationHistogram : public TFCS1DFunctionTemplateHistogram<Txvec, Ty, Trandom> {
public:
  ...
  virtual double rnd_to_fct( double rnd ) const {
    ...
    if ( dfrac > 0 ) {
      ...
      m*=l; //m is needed inside this bin, so multiply by size of this bin (matches normalization to distance between
      bins above) m/=dfrac/l; //finally, m is the slope in this bin, given that this bin was already selected. So
      overall normalize to density inside this bin
      */
    ...
    // std::cout<<"fbin="<<ibin<<" fx="<<m_HistoBorders.GetBinLowEdge(ibin)<<"
    // frac="<<m_HistoContents.get_fraction(ibin)<<" dfracprev="<<dfracprev<<" dfrac="<<dfrac<<"
    // dfracnext="<<dfracnext<<" dfracprev-dfrac="<<dfracprev-dfrac<<" dfracnext-dfrac="<<dfracnext-dfrac<<" m="<<m<<"
...
class TFCS1DFunctionInt8Int8Histogram
    : public TFCS1DFunctionTemplateHistogram<TFCS1DFunction_HistogramInt8BinEdges, uint8_t, float> {
public:
  TFCS1DFunctionInt8Int8Histogram( TH1* h = nullptr )
      : TFCS1DFunctionTemplateHistogram<TFCS1DFunction_HistogramInt8BinEdges, uint8_t, float>( h ){};

  ClassDef( TFCS1DFunctionInt8Int8Histogram, 1 ) // TFCS1DFunctionInt8Int8Histogram
};

class TFCS1DFunctionInt8Int16Histogram
    : public TFCS1DFunctionTemplateHistogram<TFCS1DFunction_HistogramInt8BinEdges, uint16_t, float> {
public:
  TFCS1DFunctionInt8Int16Histogram( TH1* h = nullptr )
      : TFCS1DFunctionTemplateHistogram<TFCS1DFunction_HistogramInt8BinEdges, uint16_t, float>( h ){};

  ClassDef( TFCS1DFunctionInt8Int16Histogram, 1 ) // TFCS1DFunctionInt8Int16Histogram
};

class TFCS1DFunctionInt8Int32Histogram
    : public TFCS1DFunctionTemplateHistogram<TFCS1DFunction_HistogramInt8BinEdges, uint32_t, float> {
public:
  TFCS1DFunctionInt8Int32Histogram( TH1* h = nullptr )
      : TFCS1DFunctionTemplateHistogram<TFCS1DFunction_HistogramInt8BinEdges, uint32_t, float>( h ){};

  ClassDef( TFCS1DFunctionInt8Int32Histogram, 1 ) // TFCS1DFunctionInt8Int32Histogram
};

class TFCS1DFunctionInt16Int16Histogram
    : public TFCS1DFunctionTemplateHistogram<TFCS1DFunction_HistogramInt16BinEdges, uint16_t, float> {
public:
  TFCS1DFunctionInt16Int16Histogram( TH1* h = nullptr )
      : TFCS1DFunctionTemplateHistogram<TFCS1DFunction_HistogramInt16BinEdges, uint16_t, float>( h ){};

  ClassDef( TFCS1DFunctionInt16Int16Histogram, 1 ) // TFCS1DFunctionInt16Int16Histogram
};

class TFCS1DFunctionInt16Int32Histogram
    : public TFCS1DFunctionTemplateHistogram<TFCS1DFunction_HistogramInt16BinEdges, uint32_t, float> {
public:
  TFCS1DFunctionInt16Int32Histogram( TH1* h = nullptr )
      : TFCS1DFunctionTemplateHistogram<TFCS1DFunction_HistogramInt16BinEdges, uint32_t, float>( h ){};
...
class TFCS1DFunctionInt8Int8InterpolationHistogram
    : public TFCS1DFunctionTemplateInterpolationHistogram<TFCS1DFunction_HistogramInt8BinEdges, uint8_t, float> {
public:
  TFCS1DFunctionInt8Int8InterpolationHistogram( TH1* h = nullptr )
      : TFCS1DFunctionTemplateInterpolationHistogram<TFCS1DFunction_HistogramInt8BinEdges, uint8_t, float>( h ){};
...
class TFCS1DFunctionInt8Int16InterpolationHistogram
    : public TFCS1DFunctionTemplateInterpolationHistogram<TFCS1DFunction_HistogramInt8BinEdges, uint16_t, float> {
public:
  TFCS1DFunctionInt8Int16InterpolationHistogram( TH1* h = nullptr )
      : TFCS1DFunctionTemplateInterpolationHistogram<TFCS1DFunction_HistogramInt8BinEdges, uint16_t, float>( h ){};
...
class TFCS1DFunctionInt16Int16InterpolationHistogram
    : public TFCS1DFunctionTemplateInterpolationHistogram<TFCS1DFunction_HistogramInt16BinEdges, uint16_t, float> {
public:
  TFCS1DFunctionInt16Int16InterpolationHistogram( TH1* h = nullptr )
      : TFCS1DFunctionTemplateInterpolationHistogram<TFCS1DFunction_HistogramInt16BinEdges, uint16_t, float>( h ){};
...
class TFCS1DFunctionInt16Int32InterpolationHistogram
    : public TFCS1DFunctionTemplateInterpolationHistogram<TFCS1DFunction_HistogramInt16BinEdges, uint32_t, float> {
public:
  TFCS1DFunctionInt16Int32InterpolationHistogram( TH1* h = nullptr )
      : TFCS1DFunctionTemplateInterpolationHistogram<TFCS1DFunction_HistogramInt16BinEdges, uint32_t, float>( h ){};
...
#  pragma link C++ class TFCS1DFunctionTemplateInterpolationHistogram < TFCS1DFunction_HistogramInt8BinEdges, uint8_t, \
      float> +                                                                                                         \
      ;
#  pragma link C++ class TFCS1DFunctionTemplateInterpolationHistogram < TFCS1DFunction_HistogramInt8BinEdges,          \
      uint16_t, float> +                                                                                               \
      ;
#  pragma link C++ class TFCS1DFunctionTemplateInterpolationHistogram < TFCS1DFunction_HistogramInt16BinEdges,         \
      uint16_t, float> +                                                                                               \
      ;
#  pragma link C++ class TFCS1DFunctionTemplateInterpolationHistogram < TFCS1DFunction_HistogramInt16BinEdges,         \
      uint32_t, float> +                                                                                               \
