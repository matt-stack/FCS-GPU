...
public:
  ...
  TFCSParametrizationChain( const TFCSParametrizationChain& ref )
      : TFCSParametrization( ref.GetName(), ref.GetTitle() ), m_chain( ref.chain() ){};

  /// Status bit for chain persistency
  enum FCSSplitChainObjects {
    kSplitChainObjects =
        BIT( 16 ) ///< Set this bit in the TObject bit field if the TFCSParametrizationBase objects in the chain should
                  ///< be written as separate keys into the root file instead of directly writing the objects. This is
                  ///< needed if the sum of all objects in the chain use >1GB of memory, which can't be handeled by
                  ///< TBuffer. Drawback is that identical objects will get stored as multiple instances
  };
...
protected:
  ...
  FCSReturnCode simulate_and_retry( TFCSParametrizationBase* parametrization, TFCSSimulationState& simulstate,
