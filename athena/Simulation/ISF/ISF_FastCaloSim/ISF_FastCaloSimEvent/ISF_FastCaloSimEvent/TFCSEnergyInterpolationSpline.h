...
public:
  ...
  void InitFromArrayInLogEkin( Int_t np, Double_t logEkin[], Double_t response[], const char* opt = nullptr,
                               Double_t valbeg = 0, Double_t valend = 0 );

  /// Initialize spline interpolation from arrays in Ekin and response=<E(reco)/Ekin(true)>
  /// opt, valbeg and valend as defined for TSpline3
  void InitFromArrayInEkin( Int_t np, Double_t Ekin[], Double_t response[], const char* opt = nullptr,
                            Double_t valbeg = 0, Double_t valend = 0 );
  ...
  static void unit_test( TFCSSimulationState* simulstate = nullptr, TFCSTruthState* truth = nullptr,
                         const TFCSExtrapolationState* extrapol = nullptr, TGraph* grspline = nullptr );
