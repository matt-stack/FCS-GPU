...
public:
  TFCS1DFunctionSpline( TH1* hist = nullptr, double maxdevgoal = 0.01, double maxeffsiggoal = 3, int maxnp = 20 ) {
    if ( hist ) Initialize( hist, maxdevgoal, maxeffsiggoal, maxnp );
  };
  ~TFCS1DFunctionSpline(){};

  static double get_maxdev( const TH1* hist, const TSpline3& sp, double& maxeffsig, double& p_maxdev,
                            double& p_maxeffsig, int ntoy = 10000 );

  virtual double Initialize( TH1* hist, double maxdevgoal = 0.01, double maxeffsiggoal = 3, int maxnp = 20 );

  virtual double InitializeFromSpline( TH1* hist, const TSpline3& sp, double maxdevgoal = 0.01,
                                       double maxeffsiggoal = 3 );
  virtual double InitializeEqualDistance( TH1* hist, double maxdevgoal = 0.01, double maxeffsiggoal = 3,
                                          int nsplinepoints = 5 );
  virtual double InitializeEqualProbability( TH1* hist, double maxdevgoal = 0.01, double maxeffsiggoal = 3,
                                             int nsplinepoints = 5 );
...
protected:
  double optimize( TSpline3& sp_best, std::vector<double>& nprop, const TH1* hist,
                   TFCS1DFunctionInt32Histogram& hist_fct, double maxdevgoal = 0.01, double maxeffsiggoal = 3 );
