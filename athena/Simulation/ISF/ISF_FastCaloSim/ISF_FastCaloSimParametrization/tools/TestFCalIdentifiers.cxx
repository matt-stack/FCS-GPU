...
void TestFCalIdentifiers( TString sampling = "Sampling_0" ) {
  ...
  //  TFile *inputFile =
  //  TFile::Open("root://eosatlas//eos/atlas/user/z/zhubacek/FastCaloSim/NTUP_110216/ISF_HitAnalysis_Zach1_merged.root");

  TFile* inputFile = TFile::Open( "/eos/atlas/user/s/schaarsc/FCS/"
  ...
  // TFile *inputFile =
  // TFile::Open("/eos/atlas/user/s/schaarsc/FCS/user.fladias.428137.FastCalo_pid11_E65536_etam35_35_zv_m100.e4001_s2865_r7736.w0_162706_matched_output.root");
  ...
  for ( Int_t ientry = 0; ientry < nEvt; ientry++ ) {
    ...
    for ( UInt_t j = 0; j < ( *vec ).size(); j++ ) {
      ...
      for ( unsigned int ihit = 0; ihit < ( ( FCS_matchedcell )( *vec )[j] ).hit.size(); ihit++ ) {
        ...
        if ( hitSampling < 21 )
          foundCell = geo->getDDE( hitSampling, hitVec->Eta(), hitVec->Phi() );
        ...
        // cout << "Hit position: " << "x: " << x << " y: " << y << " eta: " << hitVec->Eta() << " phi: " <<
        // hitVec->Phi() <<  endl;
        ...
        if ( foundCell ) { // && cell){
          ...
          if ( z > 0 )
            cout << "Side A " << hex << foundCell->identify() << " " << cell_ID << " "
                 << ( ( FCS_matchedcell )( *vec )[j] ).hit[ihit].cell_identifier << dec << " " << cell_ID << " "
                 << ( ( FCS_matchedcell )( *vec )[j] ).hit[ihit].cell_identifier << " " << ieta << " " << iphi << " "
                 << ( ieta << 16 ) + iphi << endl;
          else
            cout << "Side C " << hex << foundCell->identify() << " " << cell_ID << " "
                 << ( ( FCS_matchedcell )( *vec )[j] ).hit[ihit].cell_identifier << dec << " " << cell_ID << " "
                 << ( ( FCS_matchedcell )( *vec )[j] ).hit[ihit].cell_identifier << " " << ieta << " " << iphi << " "
                 << ( ieta << 16 ) + iphi << endl;
          // cout << cell_ID << " " << ((FCS_matchedcell)(*vec)[j]).hit[ihit].identifier << " " <<
          // ((FCS_matchedcell)(*vec)[j]).hit[ihit].cell_identifier << " " <<
          // ((FCS_matchedcell)(*vec)[j]).g4hit[ihit].identifier << " " <<
          // ((FCS_matchedcell)(*vec)[j]).g4hit[ihit].cell_identifier << endl;
