...
void wiggleClosureAndComparison( TString sampling = "Sampling_0" ) {
  ...
  geo->LoadGeometryFromFile( "/afs/cern.ch/atlas/groups/Simulation/FastCaloSim/ATLAS-GEO-20-00-01.root",
                             "ATLAS-GEO-20-00-01" );
  geo->LoadFCalGeometryFromFiles( "FCal1-electrodes.sorted.HV.09Nov2007.dat",
                                  "FCal2-electrodes.sorted.HV.April2011.dat",
                                  "FCal3-electrodes.sorted.HV.09Nov2007.dat" );

  // TFile *originalFile =
  // TFile::Open("root://eosatlas//eos/atlas/user/z/zhubacek/FastCaloSim/LArShift020715/ISF_HitAnalysis6_evgen_calo__211_E50000_50000_eta20_25_Evts0-5500_vz_0_origin_calo.merged.pool.root");
  TFile* originalFile = TFile::Open( "/eos/atlas/user/s/schaarsc/FCS/"
                                     "user.fladias.428137.FastCalo_pid11_E65536_etam35_35_zv_m100.e4001_s2864_r7736.w0_"
                                     "162706_matched_output.root/user.fladias.8834798._000001.matched_output.root" );
  ...
  */
  ...
  for ( int i = 1; i < 50; i++ ) { eff_int[i] = eff_int[i - 1] + eff_phi->GetBinContent( i + 1 ); }

  for ( int j = 0; j < 50; j++ ) { cout << eff_int[j] << endl; }
  ...
  for ( int i = 0; i < 11; i++ ) {
    ...
    dEta_dPhi[i]   = new TProfile2D( "closure_dEtadPhi_Layer_" + layerName, "closure_dEtadPhi_Layer_" + layerName,
                                   nbins_eta, -0.5, 0.5, nbins_phi, -0.5, 0.5 );
    dEta_dPhi_N[i] = new TH2F( "closure_dEtadPhi_N_Layer_" + layerName, "closure_dEtadPhi_N_Layer_" + layerName,
                               nbins_eta, -0.5, 0.5, nbins_phi, -0.5, 0.5 );

    dEta_dPhi_res[i] = new TH2F( "closure_dEtadPhi_res_Layer_" + layerName, "closure_dEtadPhi_res_Layer_" + layerName,
                                 nbins_eta, -0.5, 0.5, nbins_phi, -0.5, 0.5 );
    dEta_dPhi_N_res[i] =
        new TH2F( "closure_dEtadPhi_N_res_Layer_" + layerName, "closure_dEtadPhi_N_res_Layer_" + layerName, nbins_eta,
                  -0.5, 0.5, nbins_phi, -0.5, 0.5 );

    dEta_dPhi_avg_reco[i] =
        new TProfile2D( "dEta_dPhi_avg_reco_Layer_" + layerName, "dEta_dPhi_avg_reco_Layer_" + layerName, nbins_eta,
                        -0.5, 0.5, nbins_phi, -0.5, 0.5 );
    dEta_dPhi_avg_g4[i] = new TProfile2D( "dEta_dPhi_avg_g4_Layer_" + layerName, "dEta_dPhi_avg_g4_Layer_" + layerName,
                                          nbins_eta, -0.5, 0.5, nbins_phi, -0.5, 0.5 );
    dEta_dPhi_FCS_orig[i] =
        new TProfile2D( " dEta_dPhi_FCS_orig_Layer_" + layerName, " dEta_dPhi_FCS_orig_Layer_" + layerName, nbins_eta,
                        -0.5, 0.5, nbins_phi, -0.5, 0.5 );
    dEta_dPhi_FCS_matched[i] =
        new TProfile2D( "dEta_dPhi_FCS_matched_Layer_" + layerName, "dEta_dPhi_FCS_matched_Layer_" + layerName,
                        nbins_eta, -0.5, 0.5, nbins_phi, -0.5, 0.5 );

    dEta_dPhi_FCS_orig_overAvg[i]    = new TProfile2D( "dEta_dPhi_FCS_orig_overAvg_Layer_" + layerName,
                                                    "dEta_dPhi_FCS_orig_overAvg_reco_Layer_" + layerName, nbins_eta,
                                                    -0.5, 0.5, nbins_phi, -0.5, 0.5 );
    dEta_dPhi_FCS_matched_overAvg[i] = new TProfile2D( "dEta_dPhi_FCS_matched_overAvg_Layer_" + layerName,
                                                       "dEta_dPhi_FCS_matched_overAvg_Layer_" + layerName, nbins_eta,
                                                       -0.5, 0.5, nbins_phi, -0.5, 0.5 );

    dEta_dPhi_FCS_orig_overAvg_N[i] =
        new TH2F( "dEta_dPhi_FCS_orig_overAvg_N_Layer_" + layerName,
                  "dEta_dPhi_FCS_orig_overAvg_N_reco_Layer_" + layerName, nbins_eta, -0.5, 0.5, nbins_phi, -0.5, 0.5 );
    dEta_dPhi_FCS_matched_overAvg_N[i] =
        new TH2F( "dEta_dPhi_FCS_matched_overAvg_N_Layer_" + layerName,
                  "dEta_dPhi_FCS_matched_overAvg_N_Layer_" + layerName, nbins_eta, -0.5, 0.5, nbins_phi, -0.5, 0.5 );
  }
  ...
  for ( Int_t ientry = 0; ientry < nEvt; ientry++ ) {
    ...
    if ( ientry % 100 == 0 ) std::cout << "Processing event # " << ientry << std::endl;
    ...
    double trEta            = 0.0;
    double trPhi            = 0.0;
    // cout << "TTC_entrance_eta " << (*truth)[0].TTC_entrance_eta.size() << " " << (*truth)[0].TTC_entrance_eta[0] <<
    // endl; cout << "TTC_entrance_phi " << (*truth)[0].TTC_entrance_phi.size() << " " << (*truth)[0].TTC_entrance_phi[0]
    // << endl; cout << endl;
    ...
    for ( UInt_t j = 0; j < ( *vec ).size(); j++ ) {
      ...
      Eoriginal.insert(
          std::pair<Long64_t, double>( ( ( FCS_matchedcell )( *vec )[j] ).cell.cell_identifier, sum_energy_hit ) );
      EoriginalReco.insert( std::pair<Long64_t, double>( ( ( FCS_matchedcell )( *vec )[j] ).cell.cell_identifier,
                                                         ( ( FCS_matchedcell )( *vec )[j] ).cell.energy ) );
      EoriginalG4.insert(
          std::pair<Long64_t, double>( ( ( FCS_matchedcell )( *vec )[j] ).cell.cell_identifier, sum_energy_g4hit ) );
  ...
  for ( Int_t ientry = 0; ientry < nEvt; ientry++ ) {
    ...
    for ( UInt_t j = 0; j < ( *vec ).size(); j++ ) {
      ...
      for ( int ihit = 0; ihit < ( ( FCS_matchedcell )( *vec )[j] ).hit.size(); ihit++ ) {
        ...
        if ( hitSampling < 21 )
          someCell = geo->getDDE( hitSampling, hitVec->Eta(), hitVec->Phi() );
        else if ( hitSampling < 24 )
          someCell = geo->getFCalDDE( hitSampling, x, y, z );
        else
          someCell = 0;
        ...
        if ( Eoriginal.find( someCellID ) != Eoriginal.end() ) {
          ...
          Eoriginal.insert(
              std::pair<Long64_t, double>( someCellID, ( ( FCS_matchedcell )( *vec )[j] ).hit[ihit].hit_energy ) );
        } // end of else creating new cell
      }   // end hit loop

      // Eoriginal.insert(std::pair<Long64_t, double>(((FCS_matchedcell)(*vec)[j]).cell.cell_identifier,
      // sum_energy_hit));
    } // loop over cells
    ...
    for ( UInt_t j = 0; j < ( *vec ).size(); j++ ) {
      ...
      for ( int ihit = 0; ihit < ( ( FCS_matchedcell )( *vec )[j] ).hit.size(); ihit++ ) {
        ...
        if ( hitSampling < 21 )
          initCell = geo->getDDE( hitSampling, hitVec->Eta(), hitVec->Phi() );
        else if ( hitSampling < 24 )
          initCell = geo->getFCalDDE( hitSampling, x, y, z );
        else
          initCell = 0;
        ...
        if ( initCell && cell ) {
          ...
          if ( hitSampling < 21 )
            foundCell = geo->getDDE( hitSampling, hitVec->Eta(), ( hitVec->Phi() ) - ( wigglePhi * initCell->dphi() ) );
          else if ( hitSampling < 24 )
            foundCell = geo->getFCalDDE( hitSampling, x, y, z );
          else
            foundCell = 0;
          ...
          if ( Eclosure.find( foundCellID ) != Eclosure.end() ) {
            ...
            Eclosure.insert(
                std::pair<Long64_t, double>( foundCellID, ( ( FCS_matchedcell )( *vec )[j] ).hit[ihit].hit_energy ) );
          } // end of else creating new cell
        }   // end if cell ok
      }     // end loop over hits
