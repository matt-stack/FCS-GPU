...
void copyDir( TDirectory* src, std::string param_str ) {
  ...
  while ( ( key = (TKey*)nextkey() ) ) {
    ...
    if ( cl->InheritsFrom( "TDirectory" ) ) {
      ...
      if ( param_str.substr( 0, 1 ) == "s" )
        complete_param_str = doSParam( param_str, std::string( key->GetName() ) );
      else if ( param_str.substr( 0, 1 ) == "e" )
        complete_param_str = doEParam( param_str, std::string( key->GetName() ), std::string( src->GetPath() ) );
...
void CopyParam2File( std::string src_str, std::string param_type, unsigned int pdgid, unsigned int energy, float eta,
                     std::string dest_str = "FCSParams.root" ) {
  // ROOT-aware
  TLorentzVector* t;
  ...
  gROOT->LoadMacro(
      "../20.20.X-VAL/Simulation/ISF/ISF_FastCaloSim/ISF_FastCaloSimEvent/src/TFCS1DFunctionRegression.cxx+" );
  gROOT->LoadMacro(
      "../20.20.X-VAL/Simulation/ISF/ISF_FastCaloSim/ISF_FastCaloSimEvent/src/TFCS1DFunctionRegressionTF.cxx+" );
  gROOT->LoadMacro(
      "../20.20.X-VAL/Simulation/ISF/ISF_FastCaloSim/ISF_FastCaloSimEvent/src/TFCS1DFunctionHistogram.cxx+" );
