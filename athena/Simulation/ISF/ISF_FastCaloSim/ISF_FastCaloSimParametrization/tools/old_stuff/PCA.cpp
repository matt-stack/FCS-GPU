...
void PCA( TString InputFile = "output1" ) {
  ...
  for ( unsigned int k = 0; k < Relevant_Layers.size() + 1; k++ ) {
    ...
    if ( k == Relevant_Layers.size() ) {
      hdata[Relevant_Layers.size()] =
          new TH1F( "hdata_Total_Cell_Energy", "hdata_Total_Cell_Energy", 5000,
                    InputTree->GetMinimum( "total_cell_energy" ), InputTree->GetMaximum( "total_cell_energy" ) );
      ...
      // Get Cumulative
      hCumulative[k] =
          new TH1F( "hCumulative_Total_Cell_Energy", "hCumulative_Total_Cell_Energy", 5000,
                    InputTree->GetMinimum( "total_cell_energy" ), InputTree->GetMaximum( "total_cell_energy" ) );

    }

    else {
      hdata[k] = new TH1F( Form( "hdata%i", Relevant_Layers[k] ), Form( "hdata%i", Relevant_Layers[k] ), 5000, 0,
                           MaxInputs[Relevant_Layers[k]] );

      for ( int ientry = 0; ientry < NEntries; ientry++ ) {
        nr->GetEntry( ientry );
        hdata[k]->Fill( nr->GetVariable( Form( "cell_energy[%d]", Relevant_Layers[k] ) ) /
                        nr->GetVariable( "total_cell_energy" ) );
      }

      // Get Cumulative
      hCumulative[k] =
          new TH1F( Form( "hCumulative_Id_%i", Relevant_Layers[k] ), Form( "hCumulative_Id_%i", Relevant_Layers[k] ),
                    5000, 0, MaxInputs[Relevant_Layers[k]] );
    }
  ...
  for ( unsigned int k = 0; k < Relevant_Layers.size() + 1; k++ ) {
    ...
    if ( k == Relevant_Layers.size() ) {
      ...
      T_Uniform->Branch( Form( "data_Uniform_%i", Relevant_Layers[k] ), &data_Uniform[k],
                         Form( "data_Uniform_%i/D", Relevant_Layers[k] ) );
      T_Gauss->Branch( Form( "data_Gauss_%i", Relevant_Layers[k] ), &data_Gauss[k],
                       Form( "data_Gauss_%i/D", Relevant_Layers[k] ) );
    }
  ...
  for ( int ientry = 0; ientry < NEntries; ientry++ ) {
    ...
    for ( unsigned int k = 0; k < Relevant_Layers.size() + 1; k++ ) {
      ...
      if ( k == Relevant_Layers.size() ) {
        data[k] = nr->GetVariable( "total_cell_energy" );
      } else {
        data[k] =
            nr->GetVariable( Form( "cell_energy[%d]", Relevant_Layers[k] ) ) / nr->GetVariable( "total_cell_energy" );
      }
  ...
  for ( int ientry = 0; ientry < NEntries_Gauss; ientry++ ) {
    ...
    for ( unsigned int k = 0; k < Relevant_Layers.size() + 1; k++ ) {
      if ( k == Relevant_Layers.size() ) {
        input_data[k] = nr_Gauss->GetVariable( "data_Gauss_Total_Cell_Energy" );
      } else {
  ...
  for ( unsigned int k = 0; k < Relevant_Layers.size() + 1; k++ ) {
    ...
    hdata_PCA[k] =
        new TH1F( Form( "hdata_PCA%i", k + 1 ), Form( "hdata_PCA%i", k + 1 ), 5000,
    ...
    for ( int i = 0; i < Ratio2_1stPC; i++ ) {
      /* if (i==0) {
           hdata_PCA_Bin[k][i] = new TH1F(Form("hdata_PCA%i_bin1st_%i",k+1,i),
       Form("hdata_PCA%i_bin1st_%i",k+1,i),5000,T_PCA->GetMinimum(Form("PCA_%i",k+1)),yq[i]);
       }
       else{hdata_PCA_Bin[k][i] = new TH1F(Form("hdata_PCA%i_bin1st_%i",k+1,i),
       Form("hdata_PCA%i_bin1st_%i",k+1,i),5000,yq[i-1],yq[i]);}*/

      hdata_PCA_Bin[k][i] =
          new TH1F( Form( "hdata_PCA%i_bin1st_%i", k + 1, i + 1 ), Form( "hdata_PCA%i_bin1st_%i", k + 1, i + 1 ), 5000,
                    T_PCA->GetMinimum( Form( "PCA_%i", k + 1 ) ), T_PCA->GetMaximum( Form( "PCA_%i", k + 1 ) ) );
    }
    ...
    // Get Cumulatives
    hCumulative_PCA[k] =
        new TH1F( Form( "hCumulative_PCA_%i", k + 1 ), Form( "hCumulative_PCA_%i", k + 1 ), 5000,
                  T_PCA->GetMinimum( Form( "PCA_%i", k + 1 ) ), T_PCA->GetMaximum( Form( "PCA_%i", k + 1 ) ) );
    ...
    for ( int i = 0; i < Ratio2_1stPC; i++ ) {
      hCumulative_PCA_Bin[k][i] = new TH1F(
          Form( "hCumulative_PCA%i_bin1st_%i", k + 1, i + 1 ), Form( "hCumulative_PCA%i_bin1st_%i", k + 1, i + 1 ),
          5000, T_PCA->GetMinimum( Form( "PCA_%i", k + 1 ) ), T_PCA->GetMaximum( Form( "PCA_%i", k + 1 ) ) );

      for ( int j = 1; j <= hdata_PCA_Bin[k][i]->GetNbinsX(); j++ ) {
        hCumulative_PCA_Bin[k][i]->SetBinContent( j, hdata_PCA_Bin[k][i]->Integral( 1, j ) );
      }

      if ( hCumulative_PCA_Bin[k][i]->Integral() > 0 ) { hCumulative_PCA_Bin[k][i]->Write(); }
    }
