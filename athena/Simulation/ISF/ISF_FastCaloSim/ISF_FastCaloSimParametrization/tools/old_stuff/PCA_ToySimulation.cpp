...
/////////////////////////////////////////////////////////////////////////////////////////
void P2X( int gNVariables, TMatrixD* EigenVectors, TVectorD* MeanValues, TVectorD* SigmaValues, double* p, double* x,
          int nTest ) {
  /////////////////////////////////////////////////////////////////////////////////////////
  ...
  for ( int i = 0; i < gNVariables; i++ ) {
    x[i] = gMeanValues[i];
    for ( int j = 0; j < nTest; j++ ) x[i] += p[j] * gSigmaValues[i] * gEigenVectors[i * gNVariables + j];
  }
}
...
void PCA_ToySimulation( TString InputFile = "PCAOutput.root", TString InputFile2 = "PCA_transformation_inputs.root" ) {
  ...
  TH1F* hTotal_energy_fraction_R_Dispersion_Test =
      new TH1F( "Total_energy_fraction_R_Dispersion_Test", "Total_energy_fraction_R_Dispersion_Test", 100, 0, 2 );
  ...
  for ( unsigned int k = 0; k < Relevant_Layers.size() + 1; k++ ) {
    ...
    if ( k == Relevant_Layers.size() ) {
      ...
      /*  hdata[k] = new
        TH1F("hdata_Total_Cell_Energy","hdata_Total_Cell_Energy",50,T_Id->GetMinimum("hdata_Total_Cell_Energy"),T_Id->GetMaximum("hdata_Total_Cell_Energy"));
        hdata_R[k] = new TH1F("hdata_R_Total_Cell_Energy",
        "hdata_R_Total_Cell_Energy",50,T_Id->GetMinimum("hdata_Total_Cell_Energy"),T_Id->GetMaximum("hdata_Total_Cell_Energy"));
        */
      ...
      // Gauss Dist.
      hdata_Gauss[k]   = new TH1F( "hdata_Gauss_Total_Cell_Energy", "hdata_Gauss_Total_Cell_Energy", 50,
                                 T_Gauss->GetMinimum( "hdata_Gauss_Total_Cell_Energy" ),
                                 T_Gauss->GetMaximum( "hdata_Gauss_Total_Cell_Energy" ) );
      hdata_R_Gauss[k] = new TH1F( "hdata_R_Gauss_Total_Cell_Energy", "hdata_R_Gauss_Total_Cell_Energy", 50,
                                   T_Gauss->GetMinimum( "hdata_Gauss_Total_Cell_Energy" ),
                                   T_Gauss->GetMaximum( "hdata_Gauss_Total_Cell_Energy" ) );
      ...
      hCumulative_Id_R[k] =
          new TH1F( "hCumulative_R_Total_Cell_Energy", "hCumulative_R_Total_Cell_Energy", 50,
                    T_Id->GetMinimum( "hdata_Total_Cell_Energy" ), T_Id->GetMaximum( "hdata_Total_Cell_Energy" ) );
      hCumulative_PCA[k] = (TH1F*)input->Get( Form( "hCumulative_PCA_%i", k + 1 ) );

    }

    else { // Energy Fraction Dist.
      hdata[k]   = new TH1F( Form( "hdata%i", k + 1 ), Form( "hdata%i", k + 1 ), 50,
                           T_Id->GetMinimum( Form( "data_%i", Relevant_Layers[k] ) ),
                           T_Id->GetMaximum( Form( "data_%i", Relevant_Layers[k] ) ) );
      hdata_R[k] = new TH1F( Form( "hdata_R%i", k + 1 ), Form( "hdata_R%i", k + 1 ), 50,
                             T_Id->GetMinimum( Form( "data_%i", Relevant_Layers[k] ) ),
                             T_Id->GetMaximum( Form( "data_%i", Relevant_Layers[k] ) ) );
      ...
      // Gauss Dist.
      hdata_Gauss[k]   = new TH1F( Form( "hdata_Gauss%i", k + 1 ), Form( "hdata_Gauss%i", k + 1 ), 50,
                                 T_Gauss->GetMinimum( Form( "data_Gauss_%i", Relevant_Layers[k] ) ),
                                 T_Gauss->GetMaximum( Form( "data_Gauss_%i", Relevant_Layers[k] ) ) );
      hdata_R_Gauss[k] = new TH1F( Form( "hdata_R_Gauss%i", k + 1 ), Form( "hdata_R_Gauss%i", k + 1 ), 50,
                                   T_Gauss->GetMinimum( Form( "data_Gauss_%i", Relevant_Layers[k] ) ),
                                   T_Gauss->GetMaximum( Form( "data_Gauss_%i", Relevant_Layers[k] ) ) );
      ...
      hCumulative_Id_R[k] = new TH1F( Form( "hCumulative_Id_R%i", k + 1 ), Form( "hCumulative_Id_R%i", k + 1 ), 50,
                                      T_Id->GetMinimum( Form( "data_%i", Relevant_Layers[k] ) ),
                                      T_Id->GetMaximum( Form( "data_%i", Relevant_Layers[k] ) ) );
      hCumulative_PCA[k]  = (TH1F*)input->Get( Form( "hCumulative_PCA_%i", k + 1 ) );
    }
    ...
    // PCA Dist.
    hdata_PCA[k] =
        new TH1F( Form( "hdata_PCA%i", k + 1 ), Form( "hdata_PCA%i", k + 1 ), 50,
                  T_PCA->GetMinimum( Form( "PCA_%i", k + 1 ) ), T_PCA->GetMaximum( Form( "PCA_%i", k + 1 ) ) );
    hdata_R_PCA[k] =
        new TH1F( Form( "hdata_R_PCA%i", k + 1 ), Form( "hdata_R_PCA%i", k + 1 ), 50,
                  T_PCA->GetMinimum( Form( "PCA_%i", k + 1 ) ), T_PCA->GetMaximum( Form( "PCA_%i", k + 1 ) ) );
  ...
  for ( int i = 0; i < NPseudoExp; i++ ) {
    ...
    if ( i % 1000 == 0 ) { std::cout << i << " sumulated events" << std::endl; }

    P2X( Relevant_Layers.size() + 1, EigenVectors, MeanValues, SigmaValues, Simulated_PCA, Simulated_Gauss,
         Relevant_Layers.size() + 1 );
  ...
  std::cout << "hTotal_energy_fraction : " << hTotal_energy_fraction->GetRMS() << "+/-"
            << hTotal_energy_fraction->GetRMSError() << std::endl;
  std::cout << "hTotal_energy_fraction_R : " << hTotal_energy_fraction_R->GetRMS() << "+/-"
            << hTotal_energy_fraction_R->GetRMSError() << std::endl;
  std::cout << "hTotal_energy_fraction_R_Dispersion_Test : " << hTotal_energy_fraction_R_Dispersion_Test->GetRMS()
            << "+/-" << hTotal_energy_fraction_R_Dispersion_Test->GetRMSError() << std::endl;

  std::cout << "hTotal_Cell_energy : " << hdata[Relevant_Layers.size()]->GetMean() << "+/-"
            << hdata[Relevant_Layers.size()]->GetMeanError() << "    -    " << hdata[Relevant_Layers.size()]->GetRMS()
            << "+/-" << hdata[Relevant_Layers.size()]->GetRMSError() << std::endl;
  //   std::cout<<"hTotal_Cell_energy_R : " <<hTotal_energy_fraction_R->GetRMS() <<"+/-"
  //   <<hTotal_energy_fraction_R->GetRMSError() <<std::endl;

  std::cout << "hTotal_Cell_energy_R : " << hdata_R[Relevant_Layers.size()]->GetMean() << "+/-"
            << hdata[Relevant_Layers.size()]->GetMeanError() << "    -    " << hdata_R[Relevant_Layers.size()]->GetRMS()
            << "+/-" << hdata_R[Relevant_Layers.size()]->GetRMSError() << std::endl;
}
