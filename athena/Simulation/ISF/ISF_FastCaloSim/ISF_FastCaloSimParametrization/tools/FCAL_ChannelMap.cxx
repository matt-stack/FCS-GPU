...
//   June 2002  HMA
// ***********************************************************************
bool FCAL_ChannelMap::getTileID( int isam, float x_orig, float y_orig, int& eta, int& phi ) const {
...
float FCAL_ChannelMap::x( int isam, int eta, int phi ) const {
  ...
  if ( it != m_tileMap[isam - 1].end() ) {
    x = ( it->second ).x();
  } else { // can't find the tile, throw exception.
    char l_str[200];
    snprintf( l_str, sizeof( l_str ), "Error in FCAL_ChannelMap::x, wrong tile,phi= %d ,eta=: %d ", phi, eta );
...
void FCAL_ChannelMap::tileSize( int sam, int eta, int phi, float& dx, float& dy ) const {
  ...
  if ( it != m_tileMap[sam - 1].end() ) {
    ...
    char l_str[200];
    snprintf( l_str, sizeof( l_str ), "Error in FCAL_ChannelMap::tilesize, wrong tile,phi= %d ,eta=: %d ", phi, eta );
    std::string errorMessage( l_str );
    throw std::range_error( errorMessage.c_str() );
  }
}
...
void FCAL_ChannelMap::print_tubemap( int imap ) const {
  ...
  for ( int i = 0; i < 10; i++, it++ )
    std::cout << std::hex << it->first << "\t" << ( it->second ).get_tileName() << std::dec << "\t"
              << ( it->second ).x() << "\t" << ( it->second ).y() << std::endl;
}

// ********************** tubemap_begin **************************************
FCAL_ChannelMap::tubemap_const_iterator FCAL_ChannelMap::tubemap_begin( int imap ) const {
  return m_tubeMap[imap - 1].begin();
}

// ********************** tubemap_end ****************************************
FCAL_ChannelMap::tubemap_const_iterator FCAL_ChannelMap::tubemap_end( int imap ) const {
  return m_tubeMap[imap - 1].end();
}

// ********************** tubemap_size ***************************************
FCAL_ChannelMap::tubemap_sizetype FCAL_ChannelMap::tubemap_size( int imap ) const { return m_tubeMap[imap - 1].size(); }
