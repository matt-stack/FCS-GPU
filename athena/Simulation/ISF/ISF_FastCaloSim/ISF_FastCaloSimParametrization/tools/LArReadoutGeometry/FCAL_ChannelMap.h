...
public:
  ...
  void add_tube( const std::string& tileName, int mod, int id, int i, int j, double xCm, double yCm,
                 std::string hvFT ); // 29-03-07 include HV
...
inline FCAL_ChannelMap::TubePosition::TubePosition() : m_tileName( 0 ), m_x( 0 ), m_y( 0 ), m_hvFT( "" ) {}

inline FCAL_ChannelMap::TubePosition::TubePosition( tileName_t name, float x, float y, std::string hvFT )
    : m_tileName( name ), m_x( x ), m_y( y ), m_hvFT( hvFT ) {}

inline FCAL_ChannelMap::tileName_t FCAL_ChannelMap::TubePosition::get_tileName() const { return m_tileName; }

inline float FCAL_ChannelMap::TubePosition::x() const { return m_x; }

inline float FCAL_ChannelMap::TubePosition::y() const { return m_y; }

inline std::string FCAL_ChannelMap::TubePosition::getHVft() const { return m_hvFT; }

// Tile Position functions

inline FCAL_ChannelMap::TilePosition::TilePosition() : m_x( 0 ), m_y( 0 ), m_ntubes( 0 ) {}

inline FCAL_ChannelMap::TilePosition::TilePosition( float x, float y, int ntubes )
    : m_x( x ), m_y( y ), m_ntubes( ntubes ) {}

inline float FCAL_ChannelMap::TilePosition::x() const { return m_x; }

inline float FCAL_ChannelMap::TilePosition::y() const { return m_y; }

inline unsigned int FCAL_ChannelMap::TilePosition::ntubes() const { return m_ntubes; }

inline bool FCAL_ChannelMap::invert_x() const { return m_invert_x; }

inline bool FCAL_ChannelMap::invert_xy() const { return m_invert_xy; }
