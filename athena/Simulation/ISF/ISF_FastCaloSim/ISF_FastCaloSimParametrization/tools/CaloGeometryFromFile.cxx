...
bool CaloGeometryFromFile::LoadGeometryFromFile( TString filename, TString treename, TString hashfile ) {
  ...
  while ( 1 ) {
    ...
    if ( i % 10000 == 0 )
      cout << "Line: " << i << " line " << line << " id " << hex << id << " hash_id " << dec << hash_id << endl;
    i++;
  }
  cout << "Done." << endl;
  ...
  for ( Long64_t jentry = 0; jentry < nentries; jentry++ ) {
    ...
    if ( g_cellId_vs_cellHashId_map.find( cell.m_identify ) != g_cellId_vs_cellHashId_map.end() ) {
      ...
      cout << endl << "ERROR: Cell id not found in the cellId_vs_cellHashId_map!!!" << endl << endl;
  ...
  std::cout << "Identifier " << mcell->identify() << " sampling " << mcell->getSampling() << " eta: " << mcell->eta()
            << " phi: " << mcell->phi() << " CaloDetDescrElement=" << mcell << std::endl
            << std::endl;

  const CaloDetDescrElement* mcell2 = this->getDDE( mcell->getSampling(), mcell->eta(), mcell->phi() );
  std::cout << "Identifier " << mcell2->identify() << " sampling " << mcell2->getSampling() << " eta: " << mcell2->eta()
            << " phi: " << mcell2->phi() << " CaloDetDescrElement=" << mcell2 << std::endl
            << std::endl;
...
bool CaloGeometryFromFile::LoadFCalGeometryFromFiles( TString filename1, TString filename2, TString filename3 ) {
  ...
  for ( int imodule = 1; imodule <= 3; imodule++ ) {
    ...
    while ( 1 ) {
      ...
      m_FCal_ChannelMap.add_tube( tubeNamestring, imodule, thisTubeId, thisTubeI, thisTubeJ, thisTubeX, thisTubeY,
                                  seventh_column );
