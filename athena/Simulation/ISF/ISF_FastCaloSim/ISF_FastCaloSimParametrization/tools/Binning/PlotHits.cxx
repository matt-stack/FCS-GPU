...
int main( int argc, char** argv ) {
  ...
  Float_t     xbins[nxbins] = {-0.4,   -0.1,   -0.05,  -0.04,  -0.03, -0.02, -0.016, -0.014, -0.012, -0.01,
                           -0.008, -0.006, -0.004, -0.002, 0,     0.002, 0.004,  0.006,  0.008,  0.01,
                           0.012,  0.014,  0.016,  0.02,   0.03,  0.04,  0.05,   0.1,    0.4};
  Float_t     ybins[nybins] = {-0.4,   -0.1,   -0.05,  -0.04,  -0.03, -0.02, -0.016, -0.014, -0.012, -0.01,
                           -0.008, -0.006, -0.004, -0.002, 0,     0.002, 0.004,  0.006,  0.008,  0.01,
                           0.012,  0.014,  0.016,  0.02,   0.03,  0.04,  0.05,   0.1,    0.4};

  // POSITIVE BINNING IN DR (From Christopher's numbers)
  const Int_t nybinsPos           = 15;
  Float_t     ybinsPos[nybinsPos] = {0,     0.002, 0.004, 0.006, 0.008, 0.01, 0.012, 0.014,
                                 0.016, 0.02,  0.03,  0.04,  0.05,  0.1,  0.4};

  // REGULAR BINNIN IN ALPHA
  const Int_t nxbinsReg           = 9;
  float       rotZach             = TMath::Pi() / 8.;
  Float_t     xbinsReg[nxbinsReg] = {0 + rotZach,
                                 0.25 * TMath::Pi() + rotZach,
                                 0.5 * TMath::Pi() + rotZach,
                                 0.75 * TMath::Pi() + rotZach,
                                 TMath::Pi() + rotZach,
                                 1.25 * TMath::Pi() + rotZach,
                                 1.5 * TMath::Pi() + rotZach,
                                 1.75 * TMath::Pi() + rotZach,
                                 2 * TMath::Pi() + rotZach};
  ...
  if ( doPart == "pi" )
    // myfile =
    // "root://eosatlas.cern.ch//eos/atlas/user/z/zhubacek/FastCaloSim/NTUP_061114/OUTPUT_Merge_evgen_calo__211_E50000_50000_eta20_25_Evts0-5500_vz_0_origin_calo.pool.root/ISF_HitAnalysis_evgen_calo__211_E50000_50000_eta20_25_Evts0-5500_vz_0_origin_calo.pool.root"
    // ;
  ...
  TH2F halphadrE =
      fBinning( myfile, nxbins, xbins, nybins, ybins, nxbinsReg, xbinsReg, nybinsPos, ybinsPos, "Christopher",
                selected_layer, doPart, halpha, hdr, halphaE, hdrE, mEnergy, mxEnergy, myEnergy );
  ...
  const int nAlphaBinsVal = nAlphaBins + 1;
  Float_t   vAlphaBins[nAlphaBinsVal];
  bool      doR = false;
  ...
  while ( isEmptyBin ) {
    ...
    for ( Int_t i = 0; i < nRBinsVal; i++ ) { vRBins[i] = vRBinsSTD.at( i ); }
    ...
    TH2F halphadrE2 =
        fBinning( myfile, nxbins, xbins, nybins, ybins, nAlphaBinsVal, vAlphaBins, nRBinsVal, vRBins, "AlphaBinned",
                  selected_layer, doPart, halpha2, hdr2, halphaE2, hdrE2, mEnergy2, mxEnergy2, myEnergy2 );
    ...
    for ( Int_t i = 1; i < halphadrE2.GetNbinsX() + 1; i++ ) {
      ...
      for ( Int_t j = 1; j < halphadrE2.GetNbinsY() + 1; j++ ) {
        ...
        if ( j > 1 ) {
          float relvar = fabs(
              ( TMath::Log( halphadrE2.GetBinContent( i, j ) ) - TMath::Log( halphadrE2.GetBinContent( i, j - 1 ) ) ) /
              TMath::Log( halphadrE2.GetBinContent( i, j - 1 ) ) );
          std::cout << j << " :  " << TMath::Log( halphadrE2.GetBinContent( i, j ) ) << "  "
                    << TMath::Log( halphadrE2.GetBinContent( i, j - 1 ) ) << "  " << relvar << std::endl;

          if ( ( relvar > nrelvar1 && j < 0.5 * halphadrE2.GetNbinsY() ) ||
               ( relvar > nrelvar2 && j >= 0.5 * halphadrE2.GetNbinsY() ) ) {
            std::cout << "TOO LARGE E VARIATION BIN (" << i << "," << j << ") : " << relvar
                      << ", checking new bin number" << std::endl;
            isEmptyBin = true;
            nprob++;
          }
        }
  ...
  for ( Int_t i = 0; i < nRBinsVal; i++ ) { std::cout << i << "  " << vRBins[i] << std::endl; }
  ...
  if ( checklastbin / 0.1 > 1 ) { nRsubBins = int( checklastbin / 0.1 ) + 1; }
  ...
  while ( isEmptyBinlast ) {
    ...
    for ( Int_t i = 1; i < nRBins2last - 1; i++ ) {
      ...
      if ( doexp )
        vRBinslast[i] = vRBinslast[0] +
                        ( i * ( ( vRBinslast[nRBins2last - 1] - vRBinslast[0] ) / nRBinslast ) ) / ( nRBins2last - i );
    ...
    TH2F halphadrE3 =
        fBinning( myfile, nxbins, xbins, nybins, ybins, nAlphaBins + 1, vAlphaBins, nRBins + nRBinslast, vRBinsAll,
                  "Rlast", selected_layer, doPart, halpha3, hdr3, halphaE3, hdrE3, mEnergy3, mxEnergy3, myEnergy3 );
    ...
    for ( Int_t i = 1; i < halphadrE3.GetNbinsX() + 1; i++ ) {
      for ( Int_t j = 1; j < halphadrE3.GetNbinsY() + 1; j++ ) {
        ...
        if ( j > 1 ) {
          float relvar = fabs(
              ( TMath::Log( halphadrE3.GetBinContent( i, j ) ) - TMath::Log( halphadrE3.GetBinContent( i, j - 1 ) ) ) /
              TMath::Log( halphadrE3.GetBinContent( i, j - 1 ) ) );
          if ( ( relvar > nrelvar1 && j < 0.5 * halphadrE3.GetNbinsY() ) ||
               ( relvar > nrelvar2 && j >= 0.5 * halphadrE3.GetNbinsY() ) ) {
            std::cout << "TOO LARGE E VARIATION BIN (" << i << "," << j << ") : " << relvar
                      << ", checking new bin number" << std::endl;
            isEmptyBinlast = true;
            nproblast++;
  ...
  for ( Int_t i = 0; i < nRBinsVal - 1 + nRBinslast; i++ ) {
    if ( i < vRBinsSTD.size() - 1 ) { vRBinsSTDlast.push_back( vRBinsSTD.at( i ) ); }
    if ( i >= vRBinsSTD.size() - 1 ) {
      if ( !doexp )
        vRBinsSTDlast.push_back(
            vRBinsSTD.at( vRBinsSTD.size() - 2 ) +
            q * ( ( vRBinsSTD.at( vRBinsSTD.size() - 1 ) - vRBinsSTD.at( vRBinsSTD.size() - 2 ) ) / nRBinslast ) );
      if ( doexp )
        vRBinsSTDlast.push_back(
            vRBinsSTD.at( vRBinsSTD.size() - 2 ) +
            ( q * ( ( vRBinsSTD.at( vRBinsSTD.size() - 1 ) - vRBinsSTD.at( vRBinsSTD.size() - 2 ) ) / nRBinslast ) ) /
  ...
  for ( Int_t i = 0; i < nRBins2last; i++ ) { std::cout << i << "  " << vRBinslast[i] << std::endl; }
  ...
  TH2F halphadrE4 =
      fBinning( myfile, nxbins, xbins, nybins, ybins, nAlphaBins + 1, vAlphaBins, nRBinslast + 1, vRBinslast, "Rfinal",
                selected_layer, doPart, halpha4, hdr4, halphaE4, hdrE4, mEnergy4, mxEnergy4, myEnergy4 );
...
vector<float> fVecBin( TH1F myh, int nBins, bool doR ) {
  ...
  float TotalValue = 0;
  for ( Int_t i = 1; i < myh.GetNbinsX() + 1; i++ ) { TotalValue += myh.GetBinContent( i ); }

  const int nBins2      = nBins + 1;
  float     PerBinValue = TotalValue / nBins;

  float rotZach = 0;
  if ( !doR ) rotZach = TMath::Pi() / 8.;
  ...
  std::cout << "INTEGRAL : " << myh.GetEntries() << "  " << myh.Integral() << "  " << TotalValue << "  " << nBins
            << "  " << PerBinValue << "  " << myh.GetNbinsX() + 1 << std::endl;
  ...
  if ( doR ) { vbin.push_back( 0.4 ); }
  if ( !doR ) { vbin.push_back( 2 * TMath::Pi() + TMath::Pi() / 8. ); }
  return vbin;
}
