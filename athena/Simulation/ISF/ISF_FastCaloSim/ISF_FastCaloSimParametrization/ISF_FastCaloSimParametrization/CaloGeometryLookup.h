...
public:
  ...
  const float deta_double() { return m_deta_double; };
  const float dphi_double() { return m_dphi_double; };
...
protected:
  ...
  int raw_eta_position_to_index( float eta_raw ) const {
    return TMath::Floor( ( eta_raw - mineta_raw() ) / m_deta_double );
  };
  int raw_phi_position_to_index( float phi_raw ) const {
    return TMath::Floor( ( phi_raw - minphi_raw() ) / m_dphi_double );
  };
  bool  index_range_adjust( int& ieta, int& iphi );
  float calculate_distance_eta_phi( const CaloDetDescrElement* DDE, float eta, float phi, float& dist_eta0,
                                    float& dist_phi0 );
