...
struct SortByE {
  bool operator()( const ISF_FCS_Parametrization::FCS_StepInfo& step1,
                   const ISF_FCS_Parametrization::FCS_StepInfo& step2 ) {
    return ( step1.energy() > step2.energy() );
  }
  bool operator()( const ISF_FCS_Parametrization::FCS_StepInfo* step1,
                   const ISF_FCS_Parametrization::FCS_StepInfo* step2 ) {
    return ( step1->energy() > step2->energy() );
...
FastCaloSimParamAlg::FastCaloSimParamAlg( const std::string& name, ISvcLocator* pSvcLocator )
  ...
  declareProperty( "MinEnergy", m_minEnergy = .99, "energy border, that truncation won't cross" );
  declareProperty( "MaxRadius", m_maxRadius = 25., "maximal radius squared until two hits will be combined" );
  ...
  declareProperty( "LibStructFiles", m_lib_struct_files, "List of files to read library structures from" );
...
StatusCode FastCaloSimParamAlg::clusterize( ISF_FCS_Parametrization::FCS_StepInfoCollection* stepinfo ) const {
  ...
  for ( const auto& step : *stepinfo ) {
    if ( FCS_SIC_cells.find( step->identify() ) != FCS_SIC_cells.end() ) { // Already have a step for this cell
      ...
      FCS_SIC_cells.insert( std::pair<Identifier, ISF_FCS_Parametrization::FCS_StepInfoCollection*>(
  ...
  for ( std::map<Identifier, ISF_FCS_Parametrization::FCS_StepInfoCollection*>::iterator it = FCS_SIC_cells.begin();
        it != FCS_SIC_cells.end(); ++it ) {
    ...
    while ( it1 != ( it->second )->end() ) {
      ISF_FCS_Parametrization::FCS_StepInfoCollection::iterator it2 = it1;
      ++it2;
      while ( it2 != ( it->second )->end() ) {
        if ( ( ( *it1 )->diff2( **it2 ) < dsame ) && std::fabs( ( *it1 )->time() - ( *it2 )->time() ) < tsame ) {
          **it1 += **it2;
          it2 = ( it->second )->erase( it2 ); // also calls delete on the pointer (because the DataVector owns its
                                              // elements)
          continue;
        }
        ++it2;
  ...
  for ( const auto& step : *stepinfo ) { total_energy2 += step->energy(); }
  ...
  while ( stepIter != stepinfo->end() ) {
    ...
    ++nInvalid;
    stepIter =
        stepinfo->erase( stepIter ); // also calls delete on the pointer (because the DataVector owns its elements)
  }
