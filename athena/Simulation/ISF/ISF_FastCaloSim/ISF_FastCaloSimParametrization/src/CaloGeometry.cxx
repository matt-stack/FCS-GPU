...
CaloGeometry::CaloGeometry()
    : m_cells_in_sampling( MAX_SAMPLING )
    , m_cells_in_sampling_for_phi0( MAX_SAMPLING )
    , m_cells_in_regions( MAX_SAMPLING )
    , m_isCaloBarrel( MAX_SAMPLING )
    , m_dographs( false )
    , m_FCal_ChannelMap( 0 ) {
  // TMVA::Tools::Instance();
  for ( int i = 0; i < 2; ++i ) {
...
void CaloGeometry::PrintMapInfo( int i, int j ) {
  ...
  if ( i < 21 ) {
    ...
    cout << endl << "         ";
    cout << ", mineta_raw=" << m_cells_in_regions[i][j]->mineta_raw()
         << ", maxeta_raw=" << m_cells_in_regions[i][j]->maxeta_raw();
    cout << ", minphi_raw=" << m_cells_in_regions[i][j]->minphi_raw()
         << ", maxphi_raw=" << m_cells_in_regions[i][j]->maxphi_raw();
    cout << endl;
  } else {
    cout << ", " << m_cells_in_regions[i][j]->cell_grid_eta() << "*" << m_cells_in_regions[i][j]->cell_grid_phi();
    ...
    cout << endl << "         ";
    cout << ", minx_raw=" << m_cells_in_regions[i][j]->minx_raw()
         << ", maxx_raw=" << m_cells_in_regions[i][j]->maxx_raw();
    cout << ", miny_raw=" << m_cells_in_regions[i][j]->miny_raw()
         << ", maxy_raw=" << m_cells_in_regions[i][j]->maxy_raw();
    cout << endl;
  }
}
...
void CaloGeometry::InitRZmaps() {
  ...
  for ( unsigned int side = 0; side <= 1; ++side )
    for ( unsigned int sample = 0; sample < MAX_SAMPLING; ++sample ) {
  ...
  for ( int side = 0; side <= 1; ++side ) {
    for ( int sample = 0; sample < MAX_SAMPLING; ++sample ) {
      ...
      if ( rz_map_n[side][sample].size() > 0 ) {
        for ( FSmap<double, int>::iterator iter = rz_map_n[side][sample].begin(); iter != rz_map_n[side][sample].end();
              ++iter ) {
          double eta_raw = iter->first;
          if ( iter->second < 1 ) {
            // ATH_MSG_WARNING("rz-map for side="<<side<<" sample="<<sample<<" eta_raw="<<eta_raw<<" :
            // #cells="<<iter->second<<" !!!");
          } else {
...
TGraph* CaloGeometry::DrawGeoSampleForPhi0( int sample, int calocol, bool print ) {
  ...
  int ngr = 0;
  for ( t_eta_cellmap::iterator calo_iter = m_cells_in_sampling_for_phi0[sample].begin();
        calo_iter != m_cells_in_sampling_for_phi0[sample].end(); ++calo_iter ) {
    const CaloDetDescrElement* theDDE = ( *calo_iter ).second;
    if ( theDDE ) {
      TVector3 cv;
      ...
      if ( CaloSampling::PreSamplerB <= sample && sample <= CaloSampling::EMB3 ) { dr *= 2; }
      if ( print ) {
        cout << "sample=" << sample << " r=" << r << " dr=" << dr << " eta=" << eta << " deta=" << deta << " x=" << x
             << " y=" << y << " z=" << z << " dz=" << dz << endl;
      }
      if ( isCaloBarrel( sample ) ) {
...
TCanvas* CaloGeometry::DrawGeoForPhi0() {
  ...
  TH2D*    hcalolayout = new TH2D( "hcalolayoutPhi0", "Reconstruction geometry: calorimeter layout;z [mm];r [mm]", 50,
                                -7000, 7000, 50, 0, 4000 );
  hcalolayout->Draw();
...
const CaloDetDescrElement* CaloGeometry::getDDE( Identifier identify ) { return m_cells[identify]; }
  ...
  if ( sampling < 21 ) {
    for ( int skip_range_check = 0; skip_range_check <= 1; ++skip_range_check ) {
      for ( unsigned int j = 0; j < m_cells_in_regions[sampling].size(); ++j ) {
        ...
        if ( m_debug ) {
          cout << "CaloGeometry::getDDE : map" << j << " dist=" << newdist << " best dist=" << *distance
               << " steps=" << intsteps << endl;
        }
        if ( newdist < *distance ) {
          bestDDE   = newDDE;
...
const CaloDetDescrElement* CaloGeometry::getFCalDDE( int sampling, float x, float y, float z, float* distance,
                                                     int* steps ) {
  ...
  // cout << "CaloGeometry::getFCalDDE: x:" << x << " y: " << y << " ieta: " << ieta << " iphi: " << iphi << "
  // cmap->x(): " << m_FCal_ChannelMap.x(isam,ieta,iphi) << " cmap->y(): " << m_FCal_ChannelMap.y(isam,ieta,iphi) <<
  // endl;
  ...
  if ( distance ) { *distance = sqrt( pow( foundcell->x() - x, 2 ) + pow( foundcell->y() - y, 2 ) ); }
...
bool CaloGeometry::PostProcessGeometry() {
  for ( int i = 0; i < MAX_SAMPLING; ++i ) {
    post_process( i );
    for ( unsigned int j = 0; j < m_cells_in_regions[i].size(); ++j ) { m_cells_in_regions[i][j]->post_process(); }
    // if(i>=21) break;
  }
  ...
  for ( t_cellmap::iterator ic = m_cells.begin(); ic != m_cells.end(); ++ic ) {
    ...
    for ( int irnd = 0; irnd < nrnd; ++irnd ) {
      ...
      if ( sampling < 21 ) {
        ...
        if ( foundcell ) {
          foundcellpos << "eta=" << foundcell->eta() << " eta_raw=" << foundcell->eta_raw()
                       << " deta=" << foundcell->deta() << " ("
                       << ( foundcell->eta_raw() - foundcell->eta() ) / foundcell->deta() << ") ; "
                       << "phi=" << foundcell->phi() << " phi_raw=" << foundcell->phi_raw()
                       << " dphi=" << foundcell->dphi() << " ("
                       << ( foundcell->phi_raw() - foundcell->phi() ) / cell->dphi() << ")";
          is_inside_foundcell = TMath::Abs( ( eta - foundcell->eta() ) / foundcell->deta() ) < 0.55 &&
                                TMath::Abs( ( phi - foundcell->phi() ) / foundcell->dphi() ) < 0.55;
        }
        is_inside = TMath::Abs( ( eta - cell->eta() ) / cell->deta() ) < 0.49 &&
                    TMath::Abs( ( phi - cell->phi() ) / cell->dphi() ) < 0.49;
      } else {
        ...
        if ( foundcell ) {
          ...
          is_inside_foundcell = TMath::Abs( ( x - foundcell->x() ) / foundcell->dx() ) < 0.75 &&
                                TMath::Abs( ( y - foundcell->y() ) / foundcell->dy() ) < 0.75;
        }
        is_inside =
            TMath::Abs( ( x - cell->x() ) / cell->dx() ) < 0.49 && TMath::Abs( ( y - cell->y() ) / cell->dy() ) < 0.49;
        // m_debug=true;
      }

      if ( m_debug && foundcell ) {
        cout << "CaloGeometry::Validate(), irnd=" << irnd << ": cell id=" << cell->identify()
             << ", sampling=" << sampling << ", foundcell id=" << foundcell->identify() << ", " << steps << " steps"
             << endl;
        cout << "  " << cellpos.str() << endl;
      }
      if ( cell == foundcell ) {
        if ( steps > 3 && distance < -0.01 ) {
          cout << "cell id=" << cell->identify() << ", sampling=" << sampling << " found in " << steps
               << " steps, dist=" << distance << " " << pos.str() << endl;
          cout << "  " << cellpos.str() << endl;
        }
      } else {
        if ( !foundcell ) {
        ...
        if ( is_inside && foundcell && !is_inside_foundcell ) {
          ...
          cout << " Found instead id=" << foundcell->identify() << " in " << steps << " steps, dist=" << distance << " "
               << pos.str() << endl;
          cout << "  input sampling=" << sampling << " " << cellpos.str() << endl;
...
std::string CaloGeometry::SamplingName( int sample ) { return CaloSampling::getSamplingName( sample ); }
...
bool CaloGeometry::checkFCalGeometryConsistency() {
  ...
  for ( int imap = 1; imap <= 3; imap++ ) {
    ...
    if ( (int)m_cells_in_sampling[sampling].size() !=
         2 * std::distance( m_FCal_ChannelMap.begin( imap ), m_FCal_ChannelMap.end( imap ) ) ) {
      cout << "Error: Incompatibility between FCalChannel map and GEO file: Different number of cells in "
              "m_cells_in_sampling and FCal_ChannelMap"
           << endl;
      cout << "m_cells_in_sampling: " << m_cells_in_sampling[sampling].size() << endl;
      cout << "FCal_ChannelMap: " << 2 * std::distance( m_FCal_ChannelMap.begin( imap ), m_FCal_ChannelMap.end( imap ) )
           << endl;
      return false;
    }
