...
public:
  ...
  enum SUBPOS {
    SUBPOS_MID = TFCSExtrapolationState::SUBPOS_MID,
    SUBPOS_ENT = TFCSExtrapolationState::SUBPOS_ENT,
    SUBPOS_EXT = TFCSExtrapolationState::SUBPOS_EXT
...
protected:
  ...
  void extrapolate_to_ID( TFCSExtrapolationState& result, const TFCSTruthState* truth,
                          std::vector<Trk::HitInfo>* hitVector );
  bool get_calo_etaphi( TFCSExtrapolationState& result, std::vector<Trk::HitInfo>* hitVector, int sample,
                        int subpos = SUBPOS_MID );
  bool get_calo_surface( TFCSExtrapolationState& result, std::vector<Trk::HitInfo>* hitVector );
  bool rz_cylinder_get_calo_etaphi( std::vector<Trk::HitInfo>* hitVector, double cylR, double cylZ, Amg::Vector3D& pos,
                                    Amg::Vector3D& mom );
