...
// Note that m_xxx are pointers to vectors set to 0, not set to empty vector! see note around TBranch
{
  ...
  declareProperty( "NTruthParticles", m_NtruthParticles = 1,
...
StatusCode ISF_HitAnalysis::geoInit( IOVSVC_CALLBACK_ARGS ) {
  ...
  if ( m_larEmID == 0 ) throw std::runtime_error( "ISF_HitAnalysis: Invalid LAr EM ID helper" );
  m_larFcalID = caloIdManager->getFCAL_ID();
  if ( m_larFcalID == 0 ) throw std::runtime_error( "ISF_HitAnalysis: Invalid FCAL ID helper" );
  m_larHecID = caloIdManager->getHEC_ID();
  if ( m_larHecID == 0 ) throw std::runtime_error( "ISF_HitAnalysis: Invalid HEC ID helper" );
  ...
  if ( service( "ToolSvc", p_toolSvc ).isFailure() ) {
    ...
    if ( m_caloSurfaceHelper.retrieve().isFailure() ) ATH_MSG_INFO( "CaloSurfaceHelper not found " );
...
StatusCode ISF_HitAnalysis::initialize() {
  ...
  //#########################
  std::unique_ptr<TFile> dummyFile = std::unique_ptr<TFile>( TFile::Open(
      "dummyFile.root", "RECREATE" ) ); // This is added to suppress the error messages about memory-resident trees
  ...
  if ( m_tree ) {
    ...
    if ( m_doAllCells ) { m_tree->Branch( "AllCells", &m_oneeventcells ); }
...
StatusCode ISF_HitAnalysis::finalize() {
  ...
  std::unique_ptr<TFile> dummyGeoFile = std::unique_ptr<TFile>( TFile::Open(
      "dummyGeoFile.root", "RECREATE" ) ); // This is added to suppress the error messages about memory-resident trees
  ...
  if ( geo ) {
    ATH_MSG_INFO( "Successfull registered TTree: " << fullNtupleName );
    // this actually creates the vector itself! And only if it succeeds! Note that the result is not checked! And the
    // code is probably leaking memory in the end geo->Branch("cells",
    // &geocell,"identifier/L:eta,phi,r,eta_raw,phi_raw,r_raw,x,y,z,x_raw,y_raw,z_raw/F:Deta,Dphi,Dr,Dx,Dy,Dz/F");
  ...
  if ( m_calo_dd_man ) {
    int ncells = 0;
    for ( CaloDetDescrManager::calo_element_const_iterator calo_iter = m_calo_dd_man->element_begin();
          calo_iter < m_calo_dd_man->element_end(); ++calo_iter ) {
...
StatusCode ISF_HitAnalysis::execute() {
  ...
  if ( sc.isFailure() ) {
    ...
    for ( ISF_FCS_Parametrization::FCS_StepInfoCollection::const_iterator it = eventStepsES->begin();
          it != eventStepsES->end(); ++it ) {
  ...
  if ( sc.isFailure() ) {
    ...
    if ( mcEvent ) {
      ...
      if ( mcEvent->size() ) {
        ...
        for ( HepMC::GenEvent::particle_const_iterator it = ( *mcEvent->begin() )->particles_begin();
              it != ( *mcEvent->begin() )->particles_end(); ++it ) {
          ATH_MSG_DEBUG( "Number truth particles=" << ( *mcEvent->begin() )->particles_size()
                                                   << " loopEnd=" << loopEnd );
          particleIndex++;
          ...
          TFCSTruthState truth( ( *it )->momentum().px(), ( *it )->momentum().py(), ( *it )->momentum().pz(),
                                ( *it )->momentum().e(), ( *it )->pdg_id() );
          ...
          if ( ( *it )->production_vertex() ) {
            truth.set_vertex( ( *it )->production_vertex()->point3d().x(), ( *it )->production_vertex()->point3d().y(),
                              ( *it )->production_vertex()->point3d().z() );
          } else {
            truth.set_vertex( direction.X(), direction.Y(), direction.Z() );
            ATH_MSG_WARNING( "No particle production vetext, use VERTEX from direction: x "
                             << direction.X() << " y " << direction.Y() << " z " << direction.Z() );
          }

          if ( fabs( direction.X() - truth.vertex().X() ) > 0.1 || fabs( direction.Y() - truth.vertex().Y() ) > 0.1 ||
               fabs( direction.Z() - truth.vertex().Z() ) > 0.1 ) {
            ATH_MSG_WARNING( "VERTEX from direction: x " << direction.X() << " y " << direction.Y() << " z "
                                                         << direction.Z() );
            ATH_MSG_WARNING( "but VERTEX from hepmc: x " << truth.vertex().X() << " y " << truth.vertex().Y() << " z "
                                                         << truth.vertex().Z() );
          }
          ...
          for ( int sample = CaloCell_ID_FCS::FirstSample; sample < CaloCell_ID_FCS::MaxSample; ++sample ) {
            ...
            ATH_MSG_DEBUG( " detaBorder   ENT " << result.detaBorder( sample, 1 ) << " detaBorder   EXT "
                                                << result.detaBorder( sample, 2 ) );
    ...
    for ( TileHitVecConstIterator i_hit = hitVec->begin(); i_hit != hitVec->end(); ++i_hit ) {
      ...
      if ( m_calo_dd_man->get_element( cell_id ) ) {
        ...
        for ( int tilesubhit_i = 0; tilesubhit_i < ( *i_hit ).size(); tilesubhit_i++ ) {
          //!!
          // std::cout <<"Tile subhit: "<<tilesubhit_i<<"/"<<(*i_hit).size()<< " E:
          // "<<(*i_hit).energy(tilesubhit_i)<<std::endl;
  ...
  if ( m_doG4Hits ) {
    for ( unsigned int g4hit_i = 0; g4hit_i < m_g4hit_identifier->size(); g4hit_i++ ) {
      if ( ( *m_g4hit_sampling )[g4hit_i] >= 0 && ( *m_g4hit_sampling )[g4hit_i] <= 25 &&
           ( *m_g4hit_time )[g4hit_i] > m_TimingCut ) {
      ...
      if ( g4hits.find( ( *m_g4hit_cellidentifier )[g4hit_i] ) == g4hits.end() ) {
        ...
        if ( one_g4hit.sampling >= 12 && one_g4hit.sampling <= 20 ) { // tile
        ...
        g4hits.insert( std::pair<Long64_t, std::vector<FCS_g4hit>>( one_g4hit.cell_identifier,
                                                                    std::vector<FCS_g4hit>( 1, one_g4hit ) ) );
      } else {
  ...
  for ( unsigned int hit_i = 0; hit_i < m_hit_identifier->size(); hit_i++ ) {
    ...
    if ( hits.find( ( *m_hit_cellidentifier )[hit_i] ) == hits.end() ) {
      ...
      if ( one_hit.sampling >= 12 && one_hit.sampling <= 20 ) { // tile
      ...
      hits.insert(
          std::pair<Long64_t, std::vector<FCS_hit>>( one_hit.cell_identifier, std::vector<FCS_hit>( 1, one_hit ) ) );
    } else {
  ...
  // There could be G4hits/FCS hits for which we don't have a cell ->create a dummy empty cell with 0 energy, take the
  // cell identifier from the hit
  ...
  for ( int j = 0; j < MAX_LAYER - 1; j++ ) { m_layercells[j]->m_vector = m_oneeventcells->GetLayer( j ); }
  ...
  for ( int i = 0; i < MAX_LAYER; i++ ) {
    ...
    for ( unsigned int cellindex = 0; cellindex < m_layercells[i]->size(); cellindex++ ) {
      if ( i != MAX_LAYER - 1 ) {
        ...
        m_final_cell_energy->at( i ) +=
            m_layercells[i]->m_vector.at( cellindex ).cell.energy; // this should be here anyway
      }
...
std::vector<Trk::HitInfo>* ISF_HitAnalysis::caloHits( const HepMC::GenParticle& part ) const {
  ...
  ATH_MSG_DEBUG( "[ fastCaloSim transport ] starting transport from position eta="
                 << pos.eta() << " phi=" << pos.phi() << " d=" << pos.mag() << " pT=" << mom.perp() );
  ...
  */
  ...
  if ( m_caloEntrance && m_caloEntrance->inside( pos, 0.001 ) &&
    ...
    if ( charge == 0 ) {
      caloEntry = m_extrapolator->transportNeutralsWithPathLimit(
          inputPar, pathLim, timeLim, Trk::alongMomentum, pHypothesis, dummyHitVector, nextGeoID, m_caloEntrance );
    } else {
      caloEntry = m_extrapolator->extrapolateWithPathLimit( inputPar, pathLim, timeLim, Trk::alongMomentum, pHypothesis,
                                                            dummyHitVector, nextGeoID, m_caloEntrance );
    }
  } else
    caloEntry = &inputPar;
  ...
  if ( caloEntry ) {
    ...
    ATH_MSG_DEBUG( "[ fastCaloSim transport ] starting Calo transport from position eta="
                   << caloEntry->position().eta() << " phi=" << caloEntry->position().phi()
                   << " d=" << caloEntry->position().mag() );

    if ( charge == 0 ) {
      eParameters = m_extrapolator->transportNeutralsWithPathLimit( *caloEntry, pathLim, timeLim, Trk::alongMomentum,
                                                                    pHypothesis, hitVector, nextGeoID );
    } else {
      eParameters = m_extrapolator->extrapolateWithPathLimit( *caloEntry, pathLim, timeLim, Trk::alongMomentum,
                                                              pHypothesis, hitVector, nextGeoID );
    }
  ...
  if ( msgLvl( MSG::DEBUG ) ) {
    ...
    while ( it < hitVector->end() ) {
      ...
      ATH_MSG_DEBUG( " HIT: layer=" << sample << " sample=" << sample - 3000 << " eta=" << hitPos.eta()
                                    << " phi=" << hitPos.phi() << " d=" << hitPos.mag() );
      it++;
    }
  }
...
bool ISF_HitAnalysis::isCaloBarrel( int sample ) const { return GetCaloGeometry()->isCaloBarrel( sample ); }

double ISF_HitAnalysis::deta( int sample, double eta ) const { return GetCaloGeometry()->deta( sample, eta ); }
...
double ISF_HitAnalysis::rmid( int sample, double eta ) const { return GetCaloGeometry()->rmid( sample, eta ); }

double ISF_HitAnalysis::zmid( int sample, double eta ) const { return GetCaloGeometry()->zmid( sample, eta ); }

double ISF_HitAnalysis::rzmid( int sample, double eta ) const { return GetCaloGeometry()->rzmid( sample, eta ); }

double ISF_HitAnalysis::rent( int sample, double eta ) const { return GetCaloGeometry()->rent( sample, eta ); }

double ISF_HitAnalysis::zent( int sample, double eta ) const { return GetCaloGeometry()->zent( sample, eta ); }

double ISF_HitAnalysis::rzent( int sample, double eta ) const { return GetCaloGeometry()->rzent( sample, eta ); }

double ISF_HitAnalysis::rext( int sample, double eta ) const { return GetCaloGeometry()->rext( sample, eta ); }

double ISF_HitAnalysis::zext( int sample, double eta ) const { return GetCaloGeometry()->zext( sample, eta ); }

double ISF_HitAnalysis::rzext( int sample, double eta ) const { return GetCaloGeometry()->rzext( sample, eta ); }
