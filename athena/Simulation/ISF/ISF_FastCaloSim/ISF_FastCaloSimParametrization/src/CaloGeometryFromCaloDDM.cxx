...
bool CaloGeometryFromCaloDDM::LoadGeometryFromCaloDDM( const CaloDetDescrManager* calo_dd_man ) {
  int jentry = 0;
  for ( CaloDetDescrManager::calo_element_const_iterator calo_iter = calo_dd_man->element_begin();
        calo_iter < calo_dd_man->element_end(); ++calo_iter ) {
    const CaloDetDescrElement* pcell = *calo_iter;
    addcell( pcell );

    if ( jentry % 25000 == 0 ) { cout << jentry << " : " << pcell->getSampling() << ", " << pcell->identify() << endl; }
    ++jentry;
  }
