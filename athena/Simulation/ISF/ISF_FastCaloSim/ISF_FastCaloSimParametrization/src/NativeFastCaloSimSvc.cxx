...
ISF::NativeFastCaloSimSvc::NativeFastCaloSimSvc( const std::string& name, ISvcLocator* svc )
  ...
  declareProperty( "SimulateUndefinedBarcodeParticles", m_simulateUndefinedBCs,
  ...
  declareProperty( "BatchProcessMcTruth", m_batchProcessMcTruth = false,
...
StatusCode ISF::NativeFastCaloSimSvc::initialize() {
  ...
  if ( retrieveTools<ICaloCellMakerTool>( m_caloCellMakerTools_setup ).isFailure() ) return StatusCode::FAILURE;
  if ( retrieveTools<ICaloCellMakerTool>( m_caloCellMakerTools_simulate ).isFailure() ) return StatusCode::FAILURE;
  if ( retrieveTools<ICaloCellMakerTool>( m_caloCellMakerTools_release ).isFailure() ) return StatusCode::FAILURE;
  ...
  if ( m_caloCellHack ) {
    ATH_MSG_WARNING( m_screenOutputPrefix << " CaloCellContainer: " << m_caloCellsOutputName
...
StatusCode ISF::NativeFastCaloSimSvc::setupEvent() {
  ...
  for ( ; itrTool != endTool; ++itrTool ) {
    ...
    if ( sc.isFailure() ) { ATH_MSG_ERROR( m_screenOutputPrefix << "Error executing tool " << itrTool->name() ); }
  }
...
StatusCode ISF::NativeFastCaloSimSvc::simulate( const ISF::ISFParticle& isfp ) {
  ...
  if ( m_doPunchThrough ) {
    ...
    if ( isfpVec ) {
      ...
      for ( ; partIt != partItEnd; ++partIt ) { m_particleBroker->push( *partIt, &isfp ); }
    }
  }
...
StatusCode ISF::NativeFastCaloSimSvc::processOneParticle( const ISF::ISFParticle& isfp ) {
  ...
  for (;itrTool!=endTool;++itrTool) {
    ...
    HepMC::FourVector momentum(isfp.momentum().x(),isfp.momentum().y(),isfp.momentum().z(), sqrt(
  isfp.mass()*isfp.mass() + isfp.momentum().mag2()) ); HepMC::GenParticle* part=new
  HepMC::GenParticle(momentum,isfp.pdgCode()); HepMC::FourVector
  position(isfp.position().x(),isfp.position().y(),isfp.position().z(),isfp.timeStamp()); HepMC::GenVertex
  vertex(position); vertex.add_particle_out( part ); // HepMC::GenVertex destructor will delete the particle
    ...
    //ATH_MSG_DEBUG( m_screenOutputPrefix << "Chrono stop : delta " << m_chrono->chronoDelta
  (chronoName,IChronoStatSvc::USER ) * CLHEP::microsecond / CLHEP::second << " second " ); } //end of for-loop
  */
