...
FastCaloSimCaloExtrapolation::FastCaloSimCaloExtrapolation( const std::string& t, const std::string& n,
                                                            const IInterface* p )
    : AthAlgTool( t, n, p )
...
StatusCode FastCaloSimCaloExtrapolation::initialize() {
  ...
  ATH_MSG_INFO( "m_CaloBoundaryR=" << m_CaloBoundaryR << " m_CaloBoundaryZ=" << m_CaloBoundaryZ
                                   << " m_caloEntranceName " << m_caloEntranceName );
...
std::vector<Trk::HitInfo>* FastCaloSimCaloExtrapolation::caloHits( const TFCSTruthState* truth ) const {
  ...
  ATH_MSG_DEBUG( "[ fastCaloSim transport ] x from position eta=" << pos.eta() << " phi=" << pos.phi()
                                                                  << " d=" << pos.mag() << " pT=" << mom.perp() );
  ...
  */
  ...
  while ( it2 < hitVector->end() ) {
    ...
    ATH_MSG_DEBUG( " HIT: layer=" << sample << " sample=" << sample - 3000 << " eta=" << hitPos.eta()
                                  << " phi=" << hitPos.phi() << " r=" << hitPos.perp() << " z=" << hitPos[Amg::z] );
    it2++;
  }
...
void FastCaloSimCaloExtrapolation::extrapolate( TFCSExtrapolationState& result, const TFCSTruthState* truth,
  ...
  if ( fabs( result.IDCaloBoundary_eta() ) < 6 ) {
    ...
    ATH_MSG_DEBUG( "Calo position for particle id " << pdgid << ", trutheta= " << ptruth_eta
                                                    << ", truthphi= " << ptruth_phi << ", truthp=" << ptruth_p
                                                    << ", truthpt=" << ptruth_pt );
    for ( int sample = CaloCell_ID_FCS::FirstSample; sample < CaloCell_ID_FCS::MaxSample; ++sample ) {
      for ( int subpos = SUBPOS_MID; subpos <= SUBPOS_EXT; ++subpos ) {
        if ( get_calo_etaphi( result, hitVector, sample, subpos ) )
          ATH_MSG_DEBUG( "Result in sample "
                         << sample << "." << subpos << ": eta=" << result.eta( sample, subpos )
                         << " phi=" << result.phi( sample, subpos ) << " r=" << result.r( sample, subpos )
                         << " z=" << result.z( sample, subpos ) << " (ok=" << result.OK( sample, subpos ) << ")" );
        else
...
void FastCaloSimCaloExtrapolation::extrapolate_to_ID( TFCSExtrapolationState& result, const TFCSTruthState* /*truth*/,
                                                      std::vector<Trk::HitInfo>* hitVector ) {
  ...
  for ( unsigned int i = 0; i < m_CaloBoundaryR.size(); ++i ) {
    ...
    if ( rz_cylinder_get_calo_etaphi( hitVector, m_CaloBoundaryR[i], m_CaloBoundaryZ[i], hitpos, hitmom ) ) {
      ...
      ATH_MSG_DEBUG( "BOUNDARY ID-CALO r=" << m_CaloBoundaryR[i] << " z=" << m_CaloBoundaryZ[i]
                                           << ": eta=" << hitpos.eta() << " phi=" << hitpos.phi()
                                           << " r=" << hitpos.perp() << " z=" << hitpos[Amg::z]
                                           << " theta=" << hitpos.theta() << " ; momentum eta=" << hitmom.eta()
                                           << " phi=" << hitmom.phi() << " theta=" << hitmom.theta() );
      if ( i > 0 ) {
        if ( hitpos[Amg::z] >= 0 )
          if ( hitpos[Amg::z] < m_CaloBoundaryZ[i - 1] ) continue;
        if ( hitpos[Amg::z] < 0 )
          if ( hitpos[Amg::z] > -m_CaloBoundaryZ[i - 1] ) continue;
      }

      // test if r within next cylinder
      if ( i < m_CaloBoundaryR.size() - 1 )
        if ( hitpos.perp() < m_CaloBoundaryR[i + 1] ) continue;
  ...
  if ( result_dist < 0 ) {
    ...
    ATH_MSG_DEBUG( "FINAL BOUNDARY ID-CALO  eta="
                   << result_hitpos.eta() << " phi=" << result_hitpos.phi() << " r=" << result_hitpos.perp()
                   << " z=" << result_hitpos[Amg::z] << " theta=" << result_hitpos.theta() << " ; momentum eta="
                   << result_hitmom.eta() << " phi=" << result_hitmom.phi() << " theta=" << result_hitmom.theta() );
  }
...
bool FastCaloSimCaloExtrapolation::get_calo_surface( TFCSExtrapolationState&    result,
                                                     std::vector<Trk::HitInfo>* hitVector ) {
  ...
  if ( result.CaloSurface_sample() == CaloCell_ID_FCS::noSample ) {
    ...
    if ( TMath::Abs( result.CaloSurface_eta() ) > 4.9 || pT < 500 ||
         ( TMath::Abs( result.CaloSurface_eta() ) > 4 && pT < 1000 ) )
      ATH_MSG_DEBUG( "only entrance to calo entrance layer found, no surface : eta="
                     << result.CaloSurface_eta() << " phi=" << result.CaloSurface_phi()
                     << " r=" << result.CaloSurface_r() << " z=" << result.CaloSurface_z() << " pT=" << pT );
    else
      ATH_MSG_WARNING( "only entrance to calo entrance layer found, no surface : eta="
                       << result.CaloSurface_eta() << " phi=" << result.CaloSurface_phi()
                       << " r=" << result.CaloSurface_r() << " z=" << result.CaloSurface_z() << " pT=" << pT );
  } // sample
  else {
    ATH_MSG_DEBUG( "entrance to calo surface : sample="
                   << result.CaloSurface_sample() << " eta=" << result.CaloSurface_eta()
                   << " phi=" << result.CaloSurface_phi() << " r=" << result.CaloSurface_r()
                   << " z=" << result.CaloSurface_z() << " deta=" << min_calo_surf_dist );
  }
...
// UPDATED
bool FastCaloSimCaloExtrapolation::get_calo_etaphi( TFCSExtrapolationState&    result,
                                                    std::vector<Trk::HitInfo>* hitVector, int sample, int subpos ) {
  ...
  if ( it != hitVector->end() ) {
    ...
    ATH_MSG_DEBUG( "extrapol with layer hit for sample="
                   << sample << " : id=" << sid1 << " -> " << sid2 << " target r/z=" << best_target
                   << " r1=" << hitPos1.perp() << " z1=" << hitPos1[Amg::z] << " r2=" << hitPos2.perp()
                   << " z2=" << hitPos2[Amg::z] << " result.r=" << result.r( sample, subpos )
                   << " result.z=" << result.z( sample, subpos ) );
  }
  ...
  if ( !best_found ) {
    ...
    while ( it < hitVector->end() - 1 ) {
      ...
      ATH_MSG_DEBUG( "extrapol without layer hit for sample="
                     << sample << " : id=" << sid1 << " -> " << sid2 << " dist=" << dist << " mindist=" << best_dist
                     << " t=" << t << " best_inside=" << best_inside << " target r/z=" << tmp_target
                     << " r1=" << hitPos1.perp() << " z1=" << hitPos1[Amg::z] << " r2=" << hitPos2.perp()
                     << " z2=" << hitPos2[Amg::z] << " re=" << hitPos.perp() << " ze=" << hitPos[Amg::z]
    ...
    if ( best_found ) {
      ATH_MSG_DEBUG( "extrapol without layer hit: id=" << best_id1 << " -> " << best_id2 << " mindist=" << best_dist
                                                       ...
                                                       << " target r/z=" << best_target << " rb=" << best_hitPos.perp()
                                                       << " zb=" << best_hitPos[Amg::z] );
    }
  }
  ...
  ATH_MSG_DEBUG( "Final TTC result for sample "
                 << sample << " subpos=" << subpos << " OK() " << result.OK( sample, subpos )
                 << " eta=" << result.eta( sample, subpos ) << " phi=" << result.phi( sample, subpos )
                 << " dCalo=" << result.d( sample, subpos ) << " dist(hit)=" << hitdist );

  return result.OK( sample, subpos );
}

// UPDATED
bool FastCaloSimCaloExtrapolation::rz_cylinder_get_calo_etaphi( std::vector<Trk::HitInfo>* hitVector, double cylR,
                                                                double cylZ, Amg::Vector3D& pos, Amg::Vector3D& mom ) {
  ...
  for ( int rz = 0; rz <= 1; ++rz ) {
    ...
    while ( it < hitVector->end() - 1 ) {
      ...
      ATH_MSG_DEBUG( " extrapol without layer hit to r="
                     << cylR << " z=+-" << cylZ << " : id=" << sid1 << " -> " << sid2 << " dist=" << dist
                     << " bestdist=" << best_dist << " t=" << t << " inside=" << inside
                     << " best_inside=" << best_inside << " r1=" << hitPos1.perp() << " z1=" << hitPos1[Amg::z]
                     << " r2=" << hitPos2.perp() << " z2=" << hitPos2[Amg::z] << " re=" << hitPos.perp()
                     << " ze=" << hitPos[Amg::z] << " rb=" << best_hitPos.perp() << " zb=" << best_hitPos[Amg::z] );
    }
  }

  if ( best_found ) {
    ATH_MSG_DEBUG( " extrapol to r=" << cylR << " z=" << cylZ << ": id=" << best_id1 << " -> " << best_id2
                                     << " dist=" << best_dist << " best_inside=" << best_inside
                                     << " rb=" << best_hitPos.perp() << " zb=" << best_hitPos[Amg::z] );
  }
  pos = best_hitPos;
