...
void CaloGeometryLookup::post_process() {
  ...
  if ( sampling < 21 ) {
    ...
    if ( TMath::Abs( rneta ) > 0.05 || TMath::Abs( rnphi ) > 0.05 ) {
      cout << "ERROR: can't map cells into a regular grid for sampling " << sampling << ", map index " << index()
           << endl;
      return;
    }
    ...
    //  cout<<"Grid: Sampling "<<sampling<<"_"<<index()<<": mapping cells into a regular grid, although cells don't fit
    //  well"<<endl;
    //}
  ...
  for ( int ieta = 0; ieta < m_cell_grid_eta; ++ieta ) { m_cell_grid[ieta].resize( m_cell_grid_phi, 0 ); }
  ...
  for ( ic = m_cells.begin(); ic != m_cells.end(); ++ic ) {
    ...
    if ( index_range_adjust( ieta, iphi ) ) {
      if ( m_cell_grid[ieta][iphi] ) {
        cout << "Grid: Sampling " << sampling << "_" << index() << ": Already cell found at pos (" << ieta << ","
             << iphi << "): id=" << m_cell_grid[ieta][iphi]->identify() << "->" << refcell->identify() << endl;
      ...
      cout << "Grid: Sampling " << sampling << "_" << index() << ": Cell at pos (" << ieta << "," << iphi
           << "): id=" << refcell->identify() << " outside eta range" << endl;
    }
  }
  ...
  //  cout<<"Grid: Sampling "<<sampling<<"_"<<index()<<": "<<ncells<<"/"<<size()<<" cells filled, "<<nempty<<" empty
  //  grid positions deta="<<m_deta_double<<" dphi="<<m_dphi_double<<endl;
}

float CaloGeometryLookup::calculate_distance_eta_phi( const CaloDetDescrElement* DDE, float eta, float phi,
  ...
  while ( newDDE && nsearch < 3 ) {
    ...
    *distance = calculate_distance_eta_phi( newDDE, eta, phi, dist_eta0, dist_phi0 );
    ...
    if ( CaloGeometry::m_debug || CaloGeometry::m_debug_identify == newDDE->identify() ) {
      ...
      cout << "cell id=" << newDDE->identify() << " steps " << *steps << " steps, eta=" << eta << " phi=" << phi
           << " dist=" << *distance << " cell_grid_eta=" << cell_grid_eta() << " cell_grid_phi=" << cell_grid_phi()
           << " m_deta_double=" << m_deta_double << " m_dphi_double=" << m_dphi_double
           << " 1st_eta_corr=" << best_eta_corr << " 1st_phi_corr=" << best_phi_corr << endl;
      cout << "  input sampling=" << newDDE->getSampling() << " eta=" << newDDE->eta()
           << " eta_raw=" << newDDE->eta_raw() << " deta=" << newDDE->deta() << " ("
           << ( newDDE->eta_raw() - newDDE->eta() ) / newDDE->deta() << ") phi=" << newDDE->phi()
           << " phi_raw=" << newDDE->phi_raw() << " dphi=" << newDDE->dphi() << " ("
           << ( newDDE->phi_raw() - newDDE->phi() ) / newDDE->dphi() << ")" << endl;
    ...
    for ( int iiphi = miniphi; iiphi <= maxiphi; ++iiphi ) {
      ...
      if ( newDDE ) {
        ...
        *distance = calculate_distance_eta_phi( newDDE, eta, phi, dist_eta0, dist_phi0 );
        ...
        if ( CaloGeometry::m_debug || CaloGeometry::m_debug_identify == newDDE->identify() ) {
          cout << "CaloGeometryLookup::getDDE, windows search! minieta=" << m_mineta_correction / cell_grid_eta()
               << " maxieta=" << m_maxeta_correction / cell_grid_eta()
               << " miniphi=" << m_minphi_correction / cell_grid_phi()
               << " maxiphi=" << m_maxphi_correction / cell_grid_phi() << endl;
          cout << "cell id=" << newDDE->identify() << " steps " << *steps << " steps, eta=" << eta << " phi=" << phi
               << " dist=" << *distance << endl;
          cout << "  input sampling=" << newDDE->getSampling() << " eta=" << newDDE->eta()
               << " eta_raw=" << newDDE->eta_raw() << " deta=" << newDDE->deta() << " ("
...
void CaloGeometryLookup::CalculateTransformation()
{
  ...
  TMVA::Factory *factory = new TMVA::Factory( Form("Mapping%d_%d.root",sampling,m_index) , outputFile,
...
Form("nTrain_Regression=%d:nTest_Regression=%d:SplitMode=Random:NormMode=NumEvents:!V",(int)m_cells.size(),(int)m_cells.size())
); factory->PrepareTrainingAndTestTree( "" ,
"nTrain_Regression=0:nTest_Regression=0:SplitMode=Alternate:NormMode=NumEvents:!V" );

  factory->BookMethod( TMVA::Types::kMLP, "MLP",
"!H:!V:VarTransform=Norm:NeuronType=tanh:NCycles=20000:HiddenLayers=N+5:TestRate=6:TrainingMethod=BFGS:Sampling=0.3:SamplingEpoch=0.8:ConvergenceImprove=1e-6:ConvergenceTests=15:!UseRegulator"
);
