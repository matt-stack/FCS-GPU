...
/** Constructor **/
FastCaloSimGeometryHelper::FastCaloSimGeometryHelper( const std::string& t, const std::string& n, const IInterface* p )
    : AthAlgTool( t, n, p ), CaloGeometry(), m_geoModel( 0 ), m_caloMgr( 0 ) {
...
StatusCode FastCaloSimGeometryHelper::initialize() {
  ...
  if ( m_geoModel->geoInitialized() ) {
    ...
    if ( detStore()
             ->regFcn( &IGeoModelSvc::geoInit, m_geoModel, &FastCaloSimGeometryHelper::geoInit, this )
             .isFailure() ) {
...
bool FastCaloSimGeometryHelper::LoadGeometryFromCaloDDM() {
  ...
  int jentry = 0;
  for ( CaloDetDescrManager::calo_element_const_iterator calo_iter = m_caloMgr->element_begin();
        calo_iter < m_caloMgr->element_end(); ++calo_iter ) {
