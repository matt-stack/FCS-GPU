...
void TFCSVertexZPositionStudies::loadFiles(string dirname,string filename_nominal,vector<string>& filenames_shifted){
   
   m_dirname = dirname;
   m_file_nominal = TFile::Open((m_dirname + "/" + filename_nominal).c_str());
   if(m_file_nominal->IsZombie()){
      cout << "Error: File " << m_dirname + "/" + filename_nominal << " is zombie!" << endl;
   }
...
void TFCSVertexZPositionStudies::setParticleInfo(string filename){ // Function will attempt to determine particle info from filename
   ...
   if(pid=="22")particleType="Photons";
   else if(pid=="11")particleType="Electrons";
   else if(pid=="211")particleType="Pions";
   else particleType="";
   ...
   if(std::find(substrings.begin(),substrings.end(),"disj")!=substrings.end()){
      cout << substrings[8] << " " << substrings[9] << endl;
      double eta_low = std::stod(substrings[8])/100;
      double eta_up = std::stod(substrings[9])/100;

      if(eta_low==0.)eta=Form("%1.2f #leq #eta #leq %1.2f",-eta_up,eta_up);
      else eta=Form("%1.2f #leq #left|#eta#right| #leq %1.2f",eta_low,eta_up);
      
   }
   else {
      double eta_low = std::stod(substrings[5])/100;
      double eta_up = std::stod(substrings[6])/100;
      if(eta_low==0.)eta=Form("%1.0f #leq #eta #leq %1.2f",eta_low,eta_up);
      else eta=Form("%1.2f #leq #eta #leq %1.2f",eta_low,eta_up);
...
void TFCSVertexZPositionStudies::initializeLayersAndPCAs(){
   ...
   while( (key = (TKey*)nextkey())){
      ...
      if(find(m_layers.begin(),m_layers.end(),layer)==m_layers.end())m_layers.push_back(layer);
      if(find(m_pcas.begin(),m_pcas.end(),pca)==m_pcas.end())m_pcas.push_back(pca);
...
void TFCSVertexZPositionStudies::loadMeanEnergyHistogram(string histname){
   ...
   for(int i=0;i<npcas;i++)m_hist_nominal->GetXaxis()->SetBinLabel(i+1,Form("PCA %i",m_pcas[i]));
   ...
   for(int ipca=0;ipca<npcas;ipca++){
	 ...
	 m_hist_nominal->SetBinError(ipca+1,sqrt( pow(m_hist_nominal->GetBinError(ipca),2)+h_temp->GetStdDev() ) );
	 delete h_temp;
	 for(int ishifted=0;ishifted<m_nshifted;ishifted++){
	    h_temp = (TH1F*)m_files_shifted[ishifted]->Get(name.c_str());
	    m_histos[ishifted]->AddBinContent(ipca+1,h_temp->Integral());
	    m_histos[ishifted]->SetBinError(ipca+1,sqrt( pow(m_histos[ishifted]->GetBinError(ipca),2)+h_temp->GetStdDev() ) );	 
	    delete h_temp;
	 }
      }
   }
...
void TFCSVertexZPositionStudies::loadMeanEnergyHistogram(int pca,string histname){
   ...
   const int nlayers = m_layers.size();
   m_hist_nominal = new TH1F((m_name+"_nominal").c_str(),"",nlayers,0,nlayers);
   m_hist_nominal->Sumw2();
   m_histos.resize(m_nshifted);
   m_ratios.resize(m_nshifted);
   
   for(int i=0;i<nlayers;i++){
      m_hist_nominal->GetXaxis()->SetBinLabel(i+1,Form("Layer %i",m_layers[i]));
   }
   for(int i=0;i<m_nshifted;i++){
      m_histos[i]=(TH1F*)m_hist_nominal->Clone();
      m_ratios[i]=(TH1F*)m_hist_nominal->Clone();
...
void TFCSVertexZPositionStudies::loadHistogramsForFixedZVAndLayer(int zv_index,int layer,string histname){
   ...
   for(unsigned int i=0;i< npcas;i++){
      ...
      name+=histname;
      if(i==0)m_hist_nominal = (TH1F*)m_files_shifted[zv_index]->Get(name.c_str());
...
void TFCSVertexZPositionStudies::findBinning(bool useMMbinning,double factor,double quantile){
   ...
   auto it_low=lower_bound(binning_orig.begin(), binning_orig.end(), quantiles[0]);
   auto it_up=upper_bound(binning_orig.begin(), binning_orig.end(), quantiles[1]);
   ...
   cout << m_binning.size() << " " << m_binning[0] << " " << m_binning.back() << endl;

   TH1F* h_temp= (TH1F*)h1->Rebin(m_binning.size()-1,(TString)h1->GetName() + "_temp",&m_binning[0]);
   double stats[4];
   ...
   if(useMMbinning)bin_width_opt=factor;
   else{
      prob = {0.25,0.75};
   ...
   int Nbins_final =(m_binning[m_binning.size()-1] - m_binning[0])/bin_width_opt;
   ...
   cout << Neff << " " << (m_binning[m_binning.size()-1] - m_binning[0])/(2.*(quantiles[1] - quantiles[0])*pow(Neff,-1./3)) << " " << Nbins_final << " " << m_binning.size() << " " << (m_binning.back() - m_binning[0])/Nbins_final << endl;
...
void TFCSVertexZPositionStudies::rebinHistos(){
   ...
   for(int i=0;i<nhistos;i++){
      h_temp= (TH1F*)m_histos[i]->Rebin(m_binning.size()-1,m_hist_nominal->GetName(),&m_binning[0]);
      delete m_histos[i];
      m_histos[i]=h_temp;
...
void TFCSVertexZPositionStudies::plotHistograms(string outputDir,bool ratio_plots,bool drawErrorBars,bool useLogScale){
   
   if(ratio_plots)plotHistograms(m_ratios,outputDir,drawErrorBars,useLogScale);
   else plotHistograms(m_histos,outputDir,drawErrorBars,useLogScale);
}


void TFCSVertexZPositionStudies::plotHistograms(vector<TH1F*>& histos, string outputDir,bool drawErrorBars,bool useLogScale){
   ...
   if(useLogScale){
      ...
      for(int i=0;i<nhistos;i++)for(int ibin=1;ibin<histos[i]->GetNbinsX();ibin++){
   ...
   for(int i=0;i<nhistos;i++){
      ...
      if(i==0){
        if(useLogScale)histos[i]->GetYaxis()->SetRangeUser(min/2,max*1000);
        else histos[i]->GetYaxis()->SetRangeUser(0.5*min,max*2);
   ...
   WriteInfo((m_particle_type + ", " + m_particle_energy).c_str(), 0.04, 0.2, 0.8);
   WriteInfo((m_particle_eta).c_str(), 0.04, 0.2, 0.75);
   
   //TString figurename=outDir+Form("Ratio_layer%i_pca%i_overlay",layers[ilayer],pca);
