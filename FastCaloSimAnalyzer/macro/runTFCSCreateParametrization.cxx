...
// TFCSEnergyParametrization* NewPCAEnergyParametrization(CLHEP::HepRandomEngine *randEngine, std::string filename, int
// pdgid, int E, double etamin, double etamax)
// {
...
TFCSParametrizationBase* NewEtaChain( CLHEP::HepRandomEngine*                      randEngine,
  ...
  TString etapara_name =
      Form( "SelEta_id%d_Mom%d_%d_eta_%d_%d", pdgid, int_Mom_min, int_Mom_max, int_etamin, int_etamax );
  TString etapara_title =
      Form( "Select eta for id=%d %d<=Mom<%d %4.2f<=|eta|<%4.2f", pdgid, int_Mom_min, int_Mom_max, etamin, etamax );
  ...
  for ( ; int_etamin < int_etamax; int_etamin += 5 ) {
    ...
    TFCSParametrizationBase* para =
        FCS::NewEnergyChain( randEngine, mapping, numbersOfHits, pdgid, int_Mom_min, int_Mom_max, etamin, etamax );
    if ( para ) {
      EtaSelectChain->push_back_in_bin( para );
      cout << "=========== EtaSelectChain ==========" << endl;
      cout << "pdgid=" << pdgid << " " << int_Mom_min << "<=Mom<" << int_Mom_max << " " << etamin << "<=eta<" << etamax
           << endl;
      // para->Print("short");
      cout << "=====================================" << endl;
    } else {
      cout << "============= ERROR =================" << endl;
      cout << "ERROR: pdgid=" << pdgid << " " << int_Mom_min << "<=Mom<" << int_Mom_max << " " << etamin << "<=eta<"
           << etamax << endl;
      cout << "=====================================" << endl;
    }
  }
...
int runTFCSCreateParametrization( int pid = 22, int Emin = 64, int Emax = 4194304, float etamin = 1.0,
                                  float etamax = 1.05, std::string topDir = "TFCSParam", long seed = 42 ) {
  ...
  // TFCSParametrizationBase* para_photon = NewParametrization(randEngine,
  // "/eos/atlas/user/a/ahasib/public/Simul-FastCalo/ParametrizationPhoton/ver02/mc16_13TeV.431004.ParticleGun_pid22_E65536_disj_eta_m25_m20_20_25_zv_0.secondPCA.ver02.root",
  // "/eos/atlas/user/a/ahasib/public/Simul-FastCalo/ParametrizationPhoton/ver02/mc16_13TeV.431004.ParticleGun_pid22_E65536_disj_eta_m25_m20_20_25_zv_0.shapepara.ver02.root",
  // 22, 65536, 0.2, 0.25);

  TFCSParametrizationBase* param =
      NewEtaChain( randEngine, hit_to_cell_mapping, numbers_of_hits, pid, Emin, Emax, etamin, etamax );
