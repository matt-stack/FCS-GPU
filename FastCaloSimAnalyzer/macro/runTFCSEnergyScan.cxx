...
void runTFCSEnergyScan( int pdgid = 22, long seed = 42 ) {
  ...
  fullchainfile->ls();
  TFCSParametrizationBase* fullchain = (TFCSParametrizationBase*)fullchainfile->Get( "SelPDGID" );
  fullchainfile->Close();
  ...
  TH2D* response = new TH2D( "photon_response", "photon response", nxbin - 1, xbin, 100, 0.5, 1.5 );
  ...
  TCanvas* c = new TCanvas( "photon_response_linear_interpolation", "photon response with linear interpolation" );
  response->Draw( "colz" );
  c->SetLogx();
  c->SaveAs( ".png" );
  c->SaveAs( ".pdf" );
