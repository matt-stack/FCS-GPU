...
void LoadGpuFuncHist::LD2D() {
  ...
  */
  ...
  gpuQ(
      hipMemcpy( hf.h_bordersx, ( *m_hf2d ).h_bordersx, ( hf.nbinsx + 1 ) * sizeof( float ), hipMemcpyHostToDevice ) );
  //  std::cout << "hf.h_bordersy, pointer " <<  hf.h_bordersy  <<  std::endl ;
  gpuQ(
      hipMemcpy( hf.h_bordersy, ( *m_hf2d ).h_bordersy, ( hf.nbinsy + 1 ) * sizeof( float ), hipMemcpyHostToDevice ) );
...
void LoadGpuFuncHist::LD() {
  ...
  for ( unsigned int i = 0; i < hf.nhist; ++i ) {
    ...
    gpuQ( hipMemcpy( contents_ptr[i], ( *m_hf ).h_contents[i], h_szs[i] * sizeof( uint32_t ), hipMemcpyHostToDevice ) );
