...
__global__ void testHello() { printf( "Hello, I am from GPU thread %zu\n", hipThreadIdx_x ); }
...
__global__ void testGeo( CaloDetDescrElement* cells, GeoRegion* regions, unsigned int nregions, unsigned long ncells,
  ...
  printf( " From GPU.., region %d, cell_grid[%d][%d]: index %llu index, hashid=%lld,eta=%f, phi=%f, sample=%d , "
          "ID=%lld cell_ptr=%p \n",
...
bool GeoLoadGpu::LoadGpu() {
  ...
  if ( hipSuccess !=
       hipMemcpy( &m_cells_g[0], cells_Host, sizeof( CaloDetDescrElement ) * m_ncells, hipMemcpyHostToDevice ) )
    return false;
  std::cout << "hipMemcpy " << ii << " cells"
  ...
  if ( hipSuccess !=
       hipMemcpy( SampleIndex_g, m_sample_index_h, sizeof( Rg_Sample_Index ) * m_max_sample, hipMemcpyHostToDevice ) ) {
  ...
  if ( hipSuccess != hipMemcpy( m_regions_g, m_regions, sizeof( GeoRegion ) * m_nregions, hipMemcpyHostToDevice ) )
    return false;
