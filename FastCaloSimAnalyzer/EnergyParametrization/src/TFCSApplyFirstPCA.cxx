...
void TFCSApplyFirstPCA::init() {
  ...
  TH1D* hPCA_first_component = new TH1D( "hPCA_first_component", "hPCA_first_component", 100000, -20, 20 );
...
void TFCSApplyFirstPCA::run_over_chain( CLHEP::HepRandomEngine* randEngine, TChain* inputchain, string outfilename ) {
  ...
  vector<TH1D*> histos_data = TFCSMakeFirstPCA::get_G4_histos_from_tree( m_layer_number, read_inputTree );
  vector<TH1D*> cumul_data  = TFCSMakeFirstPCA::get_cumul_histos( m_layer_totE_name, histos_data );
  ...
  for ( int event = 0; event < read_inputTree->GetEntries(); event++ ) {
    ...
    */
    ...
    for ( unsigned int l = 0; l < m_layer_totE_name.size(); l++ ) {
      ...
      // cout<<"l "<<l<<" input_data[l] "<<input_data[l]<<" total_e "<<total_e<<endl;
    }
