...
void TFCS1DRegression::storeRegression( string weightfilename, vector<vector<double>>& fWeightMatrix0to1,
...
TH1* TFCS1DRegression::transform( TH1* h_input, float& rangeval, float& startval ) {
  ...
  if ( do_transform ) {
    ...
    for ( int b = 1; b <= nbins; b++ ) h_out->SetBinContent( b, h_input->GetBinContent( b ) );
...
int TFCS1DRegression::testHisto( TH1* hist, std::string weightfilename, float& rangeval, float& startval,
                                 std::string outfilename, int neurons_start, int neurons_end, double cut_maxdev,
                                 int ntoys ) {
  // int debug=1;

  // transform the histogram
  TH1* h_transf = transform( hist, rangeval, startval );
  h_transf->SetName( "h_transf" );
...
void TFCS1DRegression::tmvaregression_training( int neurons, TTree* regTree, std::string weightfile,
                                                std::string outfilename, int& pass_training ) {
  ...
  std::cout << std::endl;
  std::cout << "==> Start TMVARegression with " << neurons << " Neurons " << std::endl;
  ...
  TMVA::Factory* factory =
      new TMVA::Factory( "TMVARegression", outputFile, "!V:!Silent:Color:DrawProgressBar:AnalysisType=Regression" );
  ...
  // mycut,"nTrain_Regression=0:nTest_Regression=1:SplitMode=Block:NormMode=NumEvents:!V" );
  dl->PrepareTrainingAndTestTree( mycut,
                                  "nTrain_Regression=0:nTest_Regression=0:SplitMode=Alternate:NormMode=NumEvents:!V" );

  factory->BookMethod(
      dl, TMVA::Types::kMLP, "MLP",
      Form( "!H:!V:VerbosityLevel=Info:NeuronType=sigmoid:NCycles=20000:HiddenLayers=%i:TestRate=6:TrainingMethod=BFGS:"
            "Sampling=0.3:SamplingEpoch=0.8:ConvergenceImprove=1e-6:ConvergenceTests=15:!UseRegulator",
            neurons ) );
...
void TFCS1DRegression::get_weights( string weightfile, vector<vector<double>>& fWeightMatrix0to1,
                                    vector<vector<double>>& fWeightMatrix1to2 ) {
  ...
  for ( int a = 0; a < ( (TObjArray*)Network->At( 1 ) )->GetEntriesFast(); a++ ) {
    ...
    for ( int b = 0; b < ( (TObjArray*)Network->At( 0 ) )->GetEntriesFast(); b++ ) thisvector.push_back( 0 );
    fWeightMatrix0to1.push_back( thisvector );
  }

  for ( int a = 0; a < ( (TObjArray*)Network->At( 2 ) )->GetEntriesFast(); a++ ) {
    vector<double> thisvector;
    for ( int b = 0; b < ( (TObjArray*)Network->At( 1 ) )->GetEntriesFast(); b++ ) thisvector.push_back( 0 );
